...
**연습문제 10.**
공리가 **닫혀 있음**을 확인하라. $P(B) > 0$일 때 $A \mapsto P(A \mid B) = P(A \cap B)/P(B)$가 세 공리를 모두 만족함을 증명하라. 이것이 왜 중요한가?

??? success "풀이"
    **공리 1(비음성).** $P(A \cap B) \ge 0$이고 $P(B) > 0$이므로 $P(A\mid B) \ge 0$이다.
    ...
    **공리 3(가산가법성).** $A_1, A_2, \ldots$가 서로 배반이면 $A_1\cap B, A_2 \cap B, \ldots$도 서로 배반이므로

    $$
    P\!\left(\bigcup_i A_i \,\Big|\, B\right)
    ...
    N = 500_000
    # 표본공간을 6개 결과로, B 는 그중 세 개
    outcome = rng.choice(6, N, p=[.10, .15, .20, .25, .20, .10])
    B = np.isin(outcome, [1, 3, 5])

    print(f"P(B) = {B.mean():.5f}\n")
    print(f"{'결과':>5}{'P(k|B)':>12}")
    total = 0.0
    for k in range(6):
        cond = np.mean((outcome == k) & B) / B.mean()
        total += cond
        print(f"{k:>5}{cond:>12.6f}")
    print(f"{'합계':>5}{total:>12.6f}   ← 공리 2")
    print(f"\n모든 값이 0 이상인가: {all(np.mean((outcome==k)&B)/B.mean() >= 0 for k in range(6))}"
          "   ← 공리 1")
    ```
    ...
    모든 값이 0 이상인가: True   ← 공리 1
    ```

    **조건부확률이 정확히 하나의 확률측도다.** $B$ 밖의 결과에는 확률 $0$이 배정되고, $B$ 안의 결과들은 원래 비율을 유지한 채 합이 $1$이 되도록 재조정된다.

    **이것이 왜 중요한가.**

    - **모든 정리가 그대로 적용된다.** 여집합 규칙 $P(A^c \mid B) = 1 - P(A \mid B)$, 포함배제, 본페로니, 측도의 연속성이 조건부확률에서도 성립한다. **새로 증명할 필요가 없다.**
    - **조건을 중첩할 수 있다.** $P(\cdot \mid B)$가 확률측도이므로 그 안에서 다시 조건을 걸 수 있고, 그것이 $P(\cdot \mid B \cap C)$와 일치한다. 곱셈법칙과 베이즈 정리가 여기서 나온다.
    - **"정보를 갱신한다"는 것의 형식화다.** 새 정보 $B$를 얻으면 표본공간을 $B$로 좁히고 확률을 재정규화한다. 다음 절의 베이즈 정리가 이 갱신을 뒤집는 방법을 다룬다.

    **한 가지 주의.** $P(B) = 0$이면 정의되지 않는다. 연속형 확률변수에서는 $P(X = x) = 0$이므로 "$X = x$일 때"라는 조건을 이 정의로 다룰 수 없고, 조건부밀도와 정칙조건부확률이라는 별도의 장치가 필요하다. 이 절의 공리가 감당하지 못하는 첫 지점이며, 뒤에서 다시 만나게 된다. $\square$
