...
공리가 세 줄뿐이라는 것은 인색해 보이지만, 실제로 쓰는 규칙들이 전부 여기서 유도된다. 아래 여섯 가지는 확률 계산에서 거의 매번 쓰이는 것들이다.

<div class="thmbox" markdown>

### 정리 2. 공리의 따름정리 — 공집합·유한가법성·여집합·단조성·포함배제·본페로니 { .thm }

임의의 사건 $A, B, A_1, \ldots, A_n$에 대해 다음이 성립한다.

$$
\begin{aligned}
&\textbf{공집합} && P(\emptyset) = 0 \\[4pt]
...
여섯 가지가 나오는 순서가 있다. 앞의 것이 뒤의 것의 재료가 되므로 그 차례대로 보자.

**공집합이 먼저다.** 공리 목록에 $P(\emptyset) = 0$이 없다는 점에 주목하라. 넣을 필요가 없기 때문이다. $\emptyset \cap \emptyset = \emptyset$이므로 공집합은 자기 자신과도 서로소이고, 따라서

$$
\Omega = \Omega \cup \emptyset \cup \emptyset \cup \cdots
$$

는 서로소인 가산 합집합이다. 공리 (3)을 쓰면

$$
P(\Omega) = P(\Omega) + \sum_{j=1}^{\infty} P(\emptyset)
$$

이므로 $\sum_j P(\emptyset) = 0$이고, 비음성에 의해 각 항이 $0$ 이상이니 $P(\emptyset) = 0$이다.
...
이다. 마지막 등호에서 꼬리 항이 사라지는 것이 방금 증명한 $P(\emptyset) = 0$이다. **유한가법성이 가산가법성의 특수한 경우**라는 말의 내용이 이것이다.
...
- **정리 2**는 거기서 실제로 쓰는 규칙들이 전부 따라 나옴을 보였다. 공집합, 유한가법성, 여집합, 단조성, 포함배제, 본페로니. 심지어 $P(\emptyset) = 0$과 $P(A) \le 1$조차 공리가 아니라 결론이다.
