...
세 규칙은 모두 "무게" 그림에서 당연해 보이는 것들이다. 무게는 음수일 수 없고, 빈 것에는 무게가 없으며 전체를 다 합치면 1이고, 겹치지 않는 덩어리는 따로 재서 더해도 된다.
...
!!! note "$P(\emptyset) = 0$은 논리적으로는 빼도 된다"
    공리 (2)에 두 개가 들어 있는데, 앞의 $P(\emptyset) = 0$은 **나머지 공리에서 유도된다.** 따라서 공리 목록에서 빼도 확률론은 그대로 서 있고, 실제로 그것을 빼고 $P(\Omega) = 1$만 적는 책이 많다. 그럼에도 여기에 적어 두는 것은 "빈 사건에는 무게가 없다"가 공리와 나란히 놓일 만큼 기본적인 사실이기 때문이다.

    **증명.** $\emptyset \cap \emptyset = \emptyset$이므로 공집합은 **자기 자신과도 서로소**다. 따라서

    $$
    \Omega = \Omega \cup \emptyset \cup \emptyset \cup \cdots
    $$

    는 서로소인 가산 합집합이고, 공리 (3)에 의해

    $$
    P(\Omega) = P(\Omega) + \sum_{j=1}^{\infty} P(\emptyset)
    $$

    이다. $P(\Omega) = 1$은 **유한**하므로 양변에서 지울 수 있어 $\sum_j P(\emptyset) = 0$이고, 공리 (1)에 의해 각 항이 $0$ 이상이니 $P(\emptyset) = 0$이다. $\square$

    **전체 무게가 유한하다는 점이 결정적이다.** 일반 측도론에서는 $\mu(\Omega) = \infty$가 허용되어 위의 소거를 할 수 없고, 그래서 $\mu(\emptyset) = 0$을 **공리로 넣는다.** 확률에서 이것이 공짜로 따라 나오는 것은 전체 무게를 1로 못박았기 때문이다. 공리에 함께 적어 두는 편이 측도의 정의와도 모양이 맞는다.
...
집합을 넣으면 수가 나온다. 앞 절의 벽돌 그림으로 말하면 $P$는 "사건에 속한 벽돌들의 무게를 재는 저울"이고, 그래서 정의역이 $\Omega$가 아니라 $\Omega$의 **부분집합들의 모임**이다. 이 모임이 정확히 무엇이어야 하는지는 연습문제 10에서 다룬다.
...
### 정리 2. 공리의 따름정리 — 유한가법성·여집합·단조성·포함배제·본페로니 { .thm }
...
**여집합과 단조성은 유한가법성의 두 줄짜리 따름이다**(연습문제 1, 4). 특히 단조성에서 $A \subseteq \Omega$를 넣으면 $P(A) \leq 1$이 나온다. 즉 **확률이 1을 넘지 않는다는 것은 공리가 아니라 결론**이다. 비음성과 함께 쓰면 $0 \leq P(A) \leq 1$이 된다.

**포함배제**는 "겹치는 부분을 두 번 세지 말라"는 말이다. $P(A) + P(B)$는 $A \cap B$를 두 번 세므로 한 번 빼 준다(연습문제 2). 사건이 셋이면 두 번 뺀 것을 한 번 되돌려 주어야 한다.
...
자르는 위치를 뒤로 밀수록 상한과 하한이 번갈아 나오며 참값을 조여 온다. 그 일반형과 수치 확인은 연습문제 7에 있다.
...
- **정리 1**은 그 세 줄을 적었다. 음이 아니고, 빈 것은 0이며 전체는 1이고, 서로소이면 더한다. 이 가운데 $P(\emptyset) = 0$은 나머지에서 유도되므로 논리적으로는 없어도 된다.
- **정리 2**는 거기서 실제로 쓰는 규칙들이 전부 따라 나옴을 보였다. 유한가법성, 여집합, 단조성, 포함배제, 본페로니. 심지어 $P(A) \le 1$조차 공리가 아니라 결론이다.
