...
이 10,000명을 그림으로 옮기면 8.8%가 어디서 나온 수인지 한눈에 보인다.

![기저율의 지배 — 10,000명으로 세어 본 의학 검사](./img/bayes_base_rate.png)

왼쪽 나무의 두 갈래는 **밑변의 크기가 100배 다르다.** 위쪽 가지는 100명에 95%를 곱해 95명을 내놓고, 아래쪽 가지는 9,900명에 10%를 곱해 990명을 내놓는다. 곱하는 비율은 위쪽이 아홉 배 이상 크지만 밑변이 99배 작아서 결과가 뒤집힌다. 오른쪽 막대가 그 결과인 1,085명의 구성이며, 베이즈 정리의 분수 $95/1{,}085$가 바로 이 막대에서 붉은 부분이 차지하는 비율이다.

**사전확률이 하는 일이 이 밑변을 정하는 것이다.** 가능도(민감도)는 비율일 뿐이어서, 곱해질 밑변이 작으면 아무리 커도 큰 수를 만들지 못한다.
