...
![조건부확률 — 표본공간을 B로 갈아 끼운다](./img/conditional_narrowing.png)
...
![전확률의 법칙 — 분할 위의 합](./img/total_probability.png)

분할이라는 말은 상자를 빈틈없이, 겹치지 않게 나누었다는 뜻이다. 그러면 사건 $A$도 자동으로 $A \cap B_1, \ldots, A \cap B_n$으로 쪼개지고, 이들 역시 서로소이므로 가산가법성을 그대로 쓸 수 있다.

$$
P(A) = \sum_{i=1}^{n} P(A \cap B_i) = \sum_{i=1}^{n} P(A \mid B_i)\,P(B_i)
$$

첫 등호가 **분할**이고 둘째 등호가 **곱셈 규칙**이다. 전확률의 법칙은 이 둘을 이어 붙인 것에 지나지 않는다.
