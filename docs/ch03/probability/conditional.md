...
표준 52장 한 벌에서 한 장을 뽑는다. $A$ = "킹", $B$ = "그림 카드(J, Q, K)"라 하자.
...
$A$ = "합이 8", $B$ = "첫 주사위가 3"이라 하자. $P(B) = 1/6$이고 $A \cap B = \{(3,5)\}$이므로 $P(A \cap B) = 1/36$이다.
...
52장에서 비복원으로 두 장을 뽑아 둘 다 에이스일 확률은 맨 위 경로다.
...
어떤 질병이 인구의 1%에 발생한다. 검사의 민감도는 95%($P(\text{양성} \mid \text{질병}) = 0.95$), 특이도는 90%($P(\text{음성} \mid \text{질병 없음}) = 0.90$)다. 검사가 양성일 확률은 얼마인가?
