...
**보기 3.** 에이스 두 장 연속. 52장에서 비복원으로 두 장을 뽑아 둘 다 에이스일 확률은 맨 위 경로다.
...
**보기 4.** 의학 검사. 어떤 질병이 인구의 1%에 발생한다. 검사의 민감도는 95%($P(\text{양성} \mid \text{질병}) = 0.95$), 특이도는 90%($P(\text{음성} \mid \text{질병 없음}) = 0.90$)다. 검사가 양성일 확률은 얼마인가?
