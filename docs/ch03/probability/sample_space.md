...
![표본공간, 사건, 표본](./img/sample_space_event.png)
...
그럴 리 없다. 무게가 지켜야 할 **최소한의 규칙**이 있고, 그것을 세 줄로 적어 놓은 것이 다음 절의 **콜모고로프 공리**다. 지금까지 자연스럽게 써 온 성질들 — 확률은 0과 1 사이다, 빈 것은 0이고 전체는 1이다, 겹치지 않으면 더한다 — 이 바로 그 세 줄이다.
