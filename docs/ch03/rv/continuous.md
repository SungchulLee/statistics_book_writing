...
가장 널리 쓰이는 연속분포는 평균 $\mu$, 분산 $\sigma^2$인 **정규분포**이며 밀도는 다음과 같다.
...
### 보기 2. 어밀리아의 최대 대기시간 { .ex }

드라이브스루의 평균 대기시간이 $\mu = 185$초, $\sigma = 11$초인 근사 정규분포를 따른다. 어밀리아는 대기시간이 하위 10%에 드는 식당만 이용한다. 그가 받아들이는 최대 대기시간은?
