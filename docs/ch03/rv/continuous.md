...
??? success "풀이"
    ...
    **$\alpha = 1.5$면 평균은 있지만 분산이 없다.** 표본평균이 이론값 $3.0$에 맞기는 하지만, 수렴 방식이 전혀 다르다.
    ...
    |---|---|---|
    | $10^3 \to 10^4$ | $1.24$배 | $2.97$배 |
    | $10^4 \to 10^5$ | $2.40$배 | $3.29$배 |
    | $10^5 \to 10^6$ | $4.39$배 | $3.44$배 |

    **분산이 있으면 감소 배수가 $\sqrt{10} \approx 3.16$ 근처로 안정적이다.** 중심극한정리가 보장하는 $\sqrt{n}$ 속도다.

    **분산이 없으면 배수가 $1.24$에서 $4.39$까지 들쭉날쭉하다.** 이 자체가 신호다. 큰 관측 하나가 들어오느냐에 따라 표본평균이 좌우되므로, **수렴 속도라는 개념 자체가 불안정하다.** 이론적으로는 $n^{1-1/\alpha}$ 속도로 안정분포에 수렴하지만, 그 극한은 정규가 아니다.
