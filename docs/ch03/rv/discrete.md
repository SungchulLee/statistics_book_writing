...
휴고는 좋아하는 선수의 카드가 나올 때까지 팩을 사되 최대 네 팩까지만 산다. 각 팩에 그 카드가 있을 확률은 0.2다. $X$를 산 팩의 수라 하면
...
### 보기 4. 삼면체 주사위 두 개의 차이 { .ex }

$D = |D_1 - D_2|$라 하자. 아홉 개의 결과가 세 값으로 옮겨진다.
