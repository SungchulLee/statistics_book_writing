...
### 정리 1. 베리–에센 부등식 — 오차는 $O(1/\sqrt n)$ 이하
...
### 정리 2. 비정규성 척도 — 표준화 3차 적률이 속도를 정한다
...
**예: 공정한 동전.** $X_i \sim \text{Bernoulli}(0.5)$이면 $\mu = 0.5$, $\sigma^2 = 0.25$, $\rho = 0.125$이므로
...
**예: 지수분포.** $X_i \sim \text{Exponential}(1)$이면 $\mu = 1$, $\sigma^2 = 1$, $\rho = 2 + e^{-1} \approx 2.368$이므로
...
### 정리 3. 질과 양 — 중심극한정리는 무엇을, 베리–에센은 얼마나
