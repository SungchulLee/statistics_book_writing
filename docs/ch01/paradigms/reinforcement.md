...
아래는 실제로 시뮬레이션을 돌려 뽑은 청소기의 머릿속이다. `?`는 아직 가 보지 못한 곳, `#`은 벽, `.`은 더러운 것으로 파악된 곳, 공백은 청소를 마친 곳, `R`이 청소기다.
...
$t = 1$에서는 거의 전부가 물음표다. $t = 40$에서는 왼쪽 방을 대부분 파악해 청소를 마쳤고, 오른쪽은 여전히 미지의 영역이다. $t = 120$에서는 가운데 칸막이의 **문**(왼쪽에서 여섯째 줄, 벽이 뚫린 지점)을 통과해 오른쪽 방을 청소하고 있다. 지도는 청소기가 지나간 만큼만 자란다.
