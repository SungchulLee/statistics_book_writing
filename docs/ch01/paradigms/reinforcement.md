...
앞의 정식화는 상태 $s_t$를 주어진 것처럼 다루었다. 실제 문제에서 $s_t$가 무엇인지는 **설계자가 정해야 하는 선택**이며, 이 선택이 학습의 성패를 좌우한다. 아타리 게임을 배우는 심층 Q 신경망(DQN, Mnih 외, 2015)이 좋은 예다.
...
- **상태 표현**은 주어지는 것이 아니라 설계하는 것이다. 같은 문제도 날것의 화면(수만 차원)으로 볼 수도, 손으로 고른 특징(수십 차원)으로 볼 수도 있으며, 이 선택이 필요한 표본 수를 좌우한다.
