...
먼저 **좋은 자료를 얻는 일이 왜 어려운지**를 본다. 편향, 교란, 생존자 효과처럼 수집 단계에서 들어온 결함은 나중에 아무리 정교한 분석으로도 걷어낼 수 없다(1.1). 이 어려움에 대응하는 방식이 두 갈래로 갈린다. **통계학의 패러다임**은 자료를 모으기 전에 질문을 정하고 수집 과정 자체를 설계한다(1.2). **인공지능의 패러다임**은 이미 쌓여 있는 자료에서 출발해 알고리즘이 배우게 한다(1.3). 마지막으로 어느 패러다임을 택하든 마주치는 **모델 건설의 선택** — 통계 모형인가 학습 알고리즘인가, 예측인가 추론인가 — 을 다룬다(1.4).

!!! note "용어에 관하여"
    이 장의 여러 페이지는 1.2의 패러다임을 **고전적 접근**, 1.3의 패러다임을 **현대적 접근**이라고도 부른다. 같은 구분을 가리키는 다른 이름이며, 어느 쪽이 더 나은 방법이라는 뜻은 아니다. 1.4 첫머리의 **두 패러다임의 강점과 한계**가 둘을 나란히 놓고 비교한다.
...
자료를 잘 모으는 일이 왜 어려운지, 그리고 잘못 모은 자료가 어떻게 분석 전체를 무너뜨리는지:
...
- **무작위화와 눈가림** — 무작위화가 교란요인을 집단 간에 어떻게 분산시키는지, 그리고 눈가림(단일·이중·삼중)이 인간의 기대와 편향이 결과를 왜곡하는 것을 어떻게 막는지를, 표준으로 여겨지는 무작위 대조시험과 임상시험의 단계까지 포함해 상세히 설명한다.
...
세 갈래는 **알고리즘이 어떤 피드백을 받는가**로 갈린다. 지도학습은 정답 레이블을, 강화학습은 지연된 보상 신호를 받으며, 비지도학습은 아무 피드백도 받지 않는다.

### 1.4 모델 건설 방법

두 패러다임을 견주어 본 뒤, 어느 쪽을 택하든 남는 모형 건설의 선택:
...
8. 세 가지 학습 패러다임 — 지도학습, 강화학습, 비지도학습 — 은 알고리즘이 자료에서 지식을 끌어내는 주요 방식을 규정한다.
