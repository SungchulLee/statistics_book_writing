...
??? success "풀이"
    **교과서마다 다른 조건.**
    ...
    | 조건 | 출처 |
    |---|---|
    | $np_0\ge5$ 그리고 $n(1-p_0)\ge5$ | 가장 흔함 |
    | $np_0\ge10$ 그리고 $n(1-p_0)\ge10$ | 더 보수적 |
    | $np_0(1-p_0)\ge9$ | 분산 기준 |
    | $n\ge9\max\{p_0/(1-p_0),\ (1-p_0)/p_0\}$ | 왜도 기준 |
    ...
    **관찰 셋.**
    ...
    3. **정말 위험한 것은 $np_0<2$쯤**이다. 표에서 유일하게 "주의"인 행이 $p_0=0.05$, $n=20$($np_0=1$)로 0.076이다. 통상의 규칙($np_0\ge5$)이 지나치게 엄격한 셈인데, **점수 검정이 왈드보다 훨씬 강건**하기 때문이다.
...
- **타당성 조건을 확인한다.** $np_0\ge5$ 이고 $n(1-p_0)\ge5$ 가 관례이며, 그렇지 않으면 정확 이항검정을 쓴다.
