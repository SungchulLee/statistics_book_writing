...
??? success "풀이"
    **착안.** $\log S^2$의 점근분산이 $(\kappa+2)/(n-1)$이므로, 표본첨도를 넣어 보정한다.

    $$
    Z=\frac{\log S^2-\log\sigma_0^2}{\sqrt{(\hat\kappa+2)/(n-1)}}
    $$
    ...
    **왜 완전히 고쳐지지 않는가.**
    ...
    1. **$\hat\kappa$의 추정오차가 크다.** $n=30$에서 표본첨도의 표준오차가 대략 $\sqrt{24/n}=0.89$인데, 지수분포의 참 $\kappa=6$을 그 정도 오차로 추정하는 것은 매우 부정확하다.

    2. **$\hat\kappa$ 자체가 8차 적률에 의존**한다. 두꺼운 꼬리에서는 그 적률이 거의 추정 불가능하다.
    ...
    **더 나은 대안.**
    ...
    | 보넷의 절사 첨도 | 이상치의 영향을 줄인 $\hat\kappa$ |
    | 순열 | 일표본에서는 적용이 어렵다 |
