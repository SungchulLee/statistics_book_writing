...
여기서 $\hat{p}$는 표본비율, $p_0$은 가설의 모비율, $n$은 전체 관측값 수이다. $np_0 \geq 10$이고 $n(1 - p_0) \geq 10$이면 이 z-통계량은 근사적으로 표준정규분포를 따른다. 분포의 모양만 놓고 보면 문턱값 $5$인 느슨한 기준에서 이미 종 모양에 가까워지지만, 검정은 제1종 오류율이 명목 $\alpha$에 가깝기를 요구하므로 [보수적 기준](../../ch04/discrete_distributions/binomial.md#언제-쓸-수-있는가-5와-10) 쪽을 쓴다.
