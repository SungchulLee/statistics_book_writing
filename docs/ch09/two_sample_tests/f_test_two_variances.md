...
??? success "풀이"
    ...
    **첨도가 원인이다.** $S^2$의 분산은 모집단 첨도 $\kappa$에 의존한다.

    $$
    \operatorname{Var}(S^2)=\frac{\sigma^4}{n}\Bigl(\kappa-\frac{n-3}{n-1}\Bigr)
    $$

    F 분포는 정규의 $\kappa=3$을 전제로 유도됐다. 첨도가 크면 $S^2$이 실제보다 훨씬 많이 흔들리는데 F 분포는 그 흔들림을 모르므로 **과대기각**한다. 첨도가 작으면(균등의 $\kappa=1.8$) 반대로 **과소기각**한다.
