...
??? success "풀이"
    ...
    **연습문제 5의 조건($n\hat p\ge5$)을 다시 보면.** $n=20$·$p=0.1$이면 $n\hat p=2$로 조건에 미달인데, 정작 합동 $z$의 수준은 0.0392로 문제없다. **조건이 지나치게 엄격한 셈**이다. 다만 그 조건은 수준뿐 아니라 **신뢰구간의 정확성**까지 겨냥한 것이므로, 구간을 쓸 때는 여전히 유용한 지침이다.
