...
??? success "풀이"
    ...
    **원인은 큰 수끼리의 뺄셈이다.** $x_i \approx 10^9$이면 $\sum x_i^2 \approx 5\times10^{18}$이고 $(\sum x_i)^2/n$도 거의 같은 수다. 두 거대한 수를 빼서 $10$ 정도의 작은 수를 얻으려는 것인데, 배정밀도 부동소수점은 유효숫자가 약 $16$자리뿐이다. $10^{18}$ 수준의 수에서 $16$자리를 쓰면 **$10^2$ 자리 아래는 이미 표현되지 않는다.** 답이 통째로 반올림 오차에 잠긴다. 이것을 **파국적 상쇄**라 한다.
    ...
    **올바른 방법은 평균을 먼저 빼는 것이다.**
    ...
    편차 $x_i - \bar x$는 애초에 작은 수이므로 상쇄가 일어나지 않는다. 자료를 두 번 훑어야 하지만($\bar x$를 구하고, 다시 편차를 구하고) 정확하다. NumPy의 `var`가 이 방식이라 위에서 언제나 $2.5$를 준다.

    **한 번만 훑으면서도 안전한 방법이 있다.** **웰퍼드 알고리즘**은 관측을 하나씩 받으며 평균과 제곱합을 함께 갱신한다.
    ...
    마지막에 $s^2 = M_n/(n-1)$이다. 갱신식에 등장하는 것이 전부 **편차**라 큰 수의 상쇄가 없다. 자료가 스트림으로 들어와 저장할 수 없을 때 표준적으로 쓰는 방법이다.

    **이 절의 가격 자료에 대입하면.** CA 가격이 $244$ 근처이므로 $\sum x_i^2 \approx 10^5$ 수준이고, 유효숫자가 넉넉해 교과서 공식으로도 문제가 없다. **위험한 것은 값 자체가 크면서 퍼짐이 작은 자료**다.
