...
이 절은 집단 비교 그림을 실제 자료 세 가지에 적용해 보았다.

- **항공사별 지연** — 분포가 오른쪽으로 길게 늘어져 평균이 전형적인 경험을 대표하지 못한다. 중앙값과 꼬리를 함께 보아야 한다.
- **우편번호별 주택 가치** — 집단이 많을 때 어떻게 정렬하고 어디까지 보여 줄지가 그림의 성패를 가른다.
- **대출 신용등급별 소득** — 순서형 집단이므로 **범주의 자연스러운 순서를 지켜 배열해야** 한다. 빈도순 정렬이 오히려 정보를 해친다.
...
다음 절 **스트립 그림과 스웜 그림**은 요약을 거치지 않고 **점을 그대로** 찍는 방법이다. 표본이 적을 때는 이쪽이 정직하다.
