...
OLS와 WLS의 계수 추정값은 비슷하지만(기울기 1.842 대 2.011) **표준오차가 크게 다르다**. 기울기의 표준오차가 0.312에서 0.2로 줄었다.

이것이 가중회귀의 값어치다. 분산이 작은 관측값에 더 큰 가중치를 주면 같은 자료에서 더 정밀한 추정을 얻는다. OLS도 여전히 불편이지만 더 이상 최소분산은 아니다.
