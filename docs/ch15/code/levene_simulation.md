...
- 같은 대수정규 상황에서 Bartlett 검정이 $0.675$의 기각률을 보이는 것과 극명하게 대비된다(15.8절 [Bartlett 검정 비정규성 민감도](bartlett_sensitivity.md)).
...
!!! note "두 수치가 거의 같다는 사실이 핵심이다"
    ...
    $0.037$이라는 약간의 보수성은 15.5절 [로버스트 검정](../robust_tests/robust_tests.md) 연습문제 4에서 논한 자유도 문제에서 온다. 중앙값을 추정하는 데 쓴 자유도가 $F$ 기준분포에 반영되지 않기 때문이다.
...
??? success "풀이"
    ...
    **더 근본적인 이유: 필요한 적률의 차수가 낮아진다.** 15.5절 [Levene 검정](../robust_tests/levene.md) 연습문제 4에서 보았듯
