...
- **세 가지가 검정력을 정한다.** 참 분산비, 두 표본크기, 유의수준. 닫힌 형태가 복잡하므로 모의실험이 실용적이다.
- **분산비가 $1$ 에서 멀수록 검정력이 오른다.** 당연하지만, **$2$ 배 차이를 잡으려면 생각보다 큰 표본이 필요하다**는 점이 수치로 확인된다.
- **분산 검정은 평균 검정보다 검정력이 낮다.** 4차적률에 의존해 추정이 불안정하기 때문이며, 7장에서 본 "분산의 상대 정밀도가 $\sqrt{2/(n-1)}$" 이라는 사실과 이어진다.
- **표본크기 불균형이 해롭다.** 한쪽이 작으면 그 표본분산이 크게 흔들려 검정력이 떨어진다.
- **설계 단계에서 쓸 수 있다.** 원하는 검정력에 필요한 $n$ 을 모의실험으로 역산하면 된다.

다음 절부터 **바틀렛 검정**으로 넘어간다. 집단이 셋 이상인 경우다.
