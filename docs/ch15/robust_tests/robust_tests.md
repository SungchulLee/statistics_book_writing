...
왜 $F$ 검정을 대신할 것이 필요한지는 [5.9절](../../ch05/applications/var_ratio_nonnormal.md)이 표본분포 쪽에서 진단해 두었다. 요지는 이렇다. $S_1^2/S_2^2$이 $F$ 분포를 따르는 것은 정규모집단에서뿐이고, 벗어나면 어긋남의 배율이 첨도로 정해지며 **표본을 키워도 줄지 않는다.** 명목 5% 검정이 지수모집단에서 27%, 로그정규에서 49%를 기각한다. 이 장은 그 진단을 받아 **무엇을 대신 쓸 것인가**에 답한다.
