...
95% credible interval for ratio: (0.644, 3.675)
```

두 구간이 사실상 일치한다($8.842$ 대 $8.849$, $32.59$ 대 $32.64$). 확산 사전분포에서 Bayes 신용구간과 빈도주의 신뢰구간이 수렴한다는 사실을 보여준다.

분산비에 대해서는 $P(\sigma_1^2 > \sigma_2^2 \mid \text{자료}) = 0.835$이고 95% 신용구간 $(0.644, 3.675)$가 1을 포함한다. 곧 집단 1의 분산이 더 클 가능성이 높지만($83.5\%$) 결론을 내릴 만큼 확실하지는 않다.
