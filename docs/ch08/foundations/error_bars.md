...
앞의 두 절에서 신뢰구간이 무엇이고 어떻게 읽어야 하는지 보았다. 이 절은 그 구간을 **그림에 담는 방법**을 다룬다.

2장의 그림들은 자료를 보여 주었다. 오차막대는 다른 것을 보여 준다. **추정값이 얼마나 믿을 만한가.**

평균 12.5라고 말할 때, 그 12.5가 표본을 다시 뽑으면 12.4가 될 값인지 8.0이 될 값인지는 전혀 다른 이야기다. **오차막대**는 그 차이를 그림에 담는다.

그런데 오차막대에는 널리 퍼진 문제가 하나 있다. **막대가 무엇을 뜻하는지가 그림만으로는 알 수 없다는 것이다.** 같은 그림에 표준편차를 그릴 수도, 표준오차를 그릴 수도, 신뢰구간을 그릴 수도 있는데 셋의 길이가 크게 다르다.

!!! note "왜 8장에 있는가"
    오차막대는 그림이지만 **그리려면 구간을 먼저 만들어야 한다.** 표준오차, 신뢰수준, 왈드 구간, 동시 신뢰구간이 모두 이 장의 도구다. 그래서 2장의 시각화 절들이 아니라 여기에 둔다.
...
??? success "풀이"
    ...
    **함의.** [막대그림](../../ch02/visualization/bar_charts.md) 절 연습문제 9에서 "비율 막대에 신뢰구간을 붙이면 분모가 작은 범주가 스스로 경고한다"고 했는데, **그 구간이 왈드라면 경고하지 못한다.** 오히려 좁은 구간으로 안심시킨다. $\square$
...
??? success "풀이"
    ...
    **대칭 막대를 억지로 쓰면 무슨 일이 생기는가.** 아래쪽 끝이 위쪽 폭만큼 내려가는데, 그 값이 **$0$보다 작아질 수 있다.** 비는 음수가 될 수 없으므로 불가능한 구간이다. 비율의 왈드 구간이 $[0,1]$ 을 벗어나던 것과 같은 경계 문제다.
...
집단이 여럿일 때 오차막대에는 [집단 비교](../../ch02/visualization/group_comparison_examples.md) 절 연습문제 $9$의 문제가 그대로 나타난다. **동시 신뢰구간**이 왜 필요한가?
...
??? success "풀이"
    ...
    **이것이 그림에서 뜻하는 바.** 오차막대 $20$개가 그려진 그림에서 **하나쯤은 참값을 벗어나 있는 것이 정상**이다. 그런데 독자는 각 막대를 개별적으로 $95\%$ 신뢰한다고 읽으며, 특히 눈에 띄는 막대를 골라 해석한다. [집단 비교](../../ch02/visualization/group_comparison_examples.md) 절에서 본 승자의 저주가 오차막대 그림에서 재현된다.
...
??? success "풀이"
    ...
    **한 걸음 물러서서.** 오차막대는 **불확실성을 그림에 넣으려는 시도**다. 2장에서 본 다른 장치들 — [상자그림](../../ch02/visualization/boxplots.md)의 노치, [바이올린 그림](../../ch02/visualization/violin.md)의 폭, 산점도의 신뢰띠 — 이 모두 같은 목적을 갖는다.

    이 절이 보여 준 것은 **불확실성을 정직하게 전달하기가 생각보다 어렵다**는 사실이다. 요약 하나를 그리는 것은 쉽지만, 그 요약을 얼마나 믿어야 하는지까지 전달하려면 훨씬 많은 주의가 필요하다.
...
같은 함정이 **노치 상자그림**에도 있다. 노치는 중앙값의 $95\%$ 신뢰구간을 나타낸다고 하며 반폭이 $1.57\cdot\mathrm{IQR}/\sqrt{n}$이다. 이 주장을 검증하고, "노치가 겹치지 않으면 유의하다"는 판정의 실제 오류율을 구하라.
...
??? success "풀이"
    ...
    print("(1) 노치가 참 중앙값을 포함하는 비율  (광고 0.95)")
    ...
    print("\n(2) 두 집단의 참 중앙값이 같을 때 노치가 겹치지 않을 확률  (목표 0.05)")
    for n in (20, 50, 200, 1000):
        c = 0
    ...
    **(1) 포함률이 $95\%$가 아니라 약 $91\%$다.** 정규분포에서 $\operatorname{SE}(\text{중앙값})\approx 1.253\sigma/\sqrt{n}$이고 $\mathrm{IQR}\approx 1.349\sigma$이므로
    ...
    **(2) 겹침 판정의 1종 오류율은 $0.016$–$0.029$로 보수적이다.**
    ...
    **연습문제 4에서 본 것과 같은 구조다.** 두 구간이 겹치는지 보는 것은 **차이의 구간**을 보는 것과 다르며, $95\%$ 구간끼리 견주면 언제나 보수적으로 나온다.

    **다만 방향이 오차막대와 반대다.** $\pm 1$ SE 막대는 오류율을 $0.165$로 **부풀렸는데**, 노치는 $0.02$ 수준으로 **줄인다.** 어느 쪽이든 명목 $0.05$가 아니다.

    | 시각적 판정 | 실제 1종 오류율 |
    |---|---|
    | $\pm 1$ SE 막대 비겹침 | $0.165$ (**세 배 관대**) |
    | $95\%$ CI 비겹침 | $0.005$ 수준 (**보수적**) |
    | 노치 비겹침 | $0.016$–$0.029$ (**보수적**) |

    **실무적 해석.**

    - **노치가 겹치지 않으면** 차이가 있다고 볼 만하다. 보수적이므로 이 방향은 안전하다.
    - **노치가 겹친다고 차이가 없는 것은 아니다.** 검정력이 낮아 진짜 차이를 놓친다.
    - **형식적 결론이 필요하면 검정을 하라.** 노치는 탐색용 보조 장치다.

    노치가 $n$이 작을 때 상자를 넘어 **모래시계 모양**이 되는 현상은 [상자그림](../../ch02/visualization/boxplots.md) 절 연습문제 9에서 다룬다. $\square$
