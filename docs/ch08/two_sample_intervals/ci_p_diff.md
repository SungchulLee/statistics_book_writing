...
구간의 포함확률이 명목값에 가깝게 나오려면 두 집단이 각각 [보수적 기준](../../ch04/discrete_distributions/binomial.md#언제-쓸-수-있는가-5와-10)을 만족해야 한다.

- $n_1\hat{p}_1 \ge 10$이고 $n_1(1 - \hat{p}_1) \ge 10$,
- $n_2\hat{p}_2 \ge 10$이고 $n_2(1 - \hat{p}_2) \ge 10$.

분포의 모양만 보려면 문턱값 $5$인 느슨한 기준으로도 족하지만, 구간을 다루므로 여기서는 $10$으로 올려 잡는다. 게다가 두 집단의 오차가 더해지는 자리라 한쪽만 아슬아슬해도 구간 전체가 흔들린다.
...
??? success "풀이"
    구간을 다루므로 보수적 기준 $n_i\hat{p}_i \geq 10$이고 $n_i(1-\hat{p}_i) \geq 10$을 본다:

    - 표본 1: $200 \times 0.75 = 150 \geq 10$이고 $200 \times 0.25 = 50 \geq 10$
    - 표본 2: $180 \times 0.667 = 120 \geq 10$이고 $180 \times 0.333 = 60 \geq 10$
...
- **타당성 조건을 확인해야 한다.** 각 집단에서 성공과 실패가 모두 충분히 있어야 정규근사가 성립한다. 구간의 포함확률을 따지는 자리이므로 보수적 기준 $n\hat p\ge10$, $n(1-\hat p)\ge10$ 을 본다.
