...
??? success "풀이"
    ...
    **앞서 본 단일 분산 구간과 같은 구조다.** $\log(S_1^2/S_2^2)$의 점근분산이
    ...
    $$
    \operatorname{Var}\left\{\log\frac{S_1^2}{S_2^2}\right\}\approx(\kappa+2)\left(\frac1{n_1}+\frac1{n_2}\right)
    $$

    로 초과첨도 $\kappa$에 비례하는데, $F$ 분포는 $\kappa=0$을 가정한다. 지수분포($\kappa=6$)에서는 **참 분산이 네 배**라 구간이 절반 폭밖에 안 된다.

    **꼬리가 얇으면 반대다.** 균등분포($\kappa=-1.2$)에서 0.994로 지나치게 보수적이다.
...
??? success "풀이"
    ...
    **부트스트랩이 조금 더 낫다.** 첨도를 명시적으로 추정하지 않고 재표본 분포가 스스로 말하게 하기 때문이다. 첨도 보정은 $\hat\kappa$의 추정오차가 크다는 약점을 그대로 안고 있다.
