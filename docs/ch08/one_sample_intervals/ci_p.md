...
비율의 표본분포가 근사적으로 정규가 되도록 표본크기 $n$이 충분히 커야 한다. 여기 적은 $10$은 **보수적 기준**이다. 분포의 모양만 보고 확률 하나를 어림할 참이면 문턱값 $5$인 느슨한 기준으로 족하지만, 구간을 만드는 일은 요구 수준이 다르다. 명목 95%를 약속한 구간이 실제로도 95% 가까이 참값을 담기를 바라기 때문이다. 두 기준을 가르는 근거는 [이항분포의 정규근사](../../ch04/discrete_distributions/binomial.md#언제-쓸-수-있는가-5와-10)에 정리해 두었다.
