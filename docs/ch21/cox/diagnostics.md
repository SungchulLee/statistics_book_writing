...
- **쇤펠트 잔차가 가장 중요하다.** 시간에 대해 추세가 보이면 위험비가 일정하지 않다는 뜻이며, 기울기 검정으로 형식화된다.
- **비례위험이 깨지면 대안이 있다.** 시간 의존 계수, 층화 콕스 모형(층마다 $h_0$ 를 따로 둠), 또는 기간을 나눠 분석한다.
- **마팅게일 잔차를 공변량에 대해 그리면** 곡선 패턴이 보일 때 변환이나 스플라인이 필요함을 알 수 있다. 13장의 부분잔차 그림과 같은 구실이다.
- **표본이 크면 쇤펠트 검정이 과민하다.** 다른 가정 검정과 마찬가지이며, 그림과 함께 판단한다.
- **진단 없이 위험비를 해석하지 않는다.** 가정이 깨진 상태의 HR 은 기간 평균에 지나지 않는다.

다음 절 **콕스 모형 실습**으로 넘어간다.
