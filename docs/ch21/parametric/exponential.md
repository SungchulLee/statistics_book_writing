...
지수 모형은 **위험이 일정**하다고 가정한다.

$$
h(t)=\lambda,\qquad S(t)=e^{-\lambda t}
$$

- **무기억성의 생존분석판이다.** 이미 얼마나 오래 버텼는지가 앞으로의 위험에 아무 영향을 주지 않으며, 4장의 지수분포 성질 그대로다.
- **모수가 하나뿐이라 가장 단순하다.** 기준선 모형으로 유용하며, 더 복잡한 모형의 비교 대상이 된다.
- **가정이 강하다.** 노화하는 대상(기계 마모, 질병 진행)이나 초기 고장이 있는 대상에는 맞지 않는다.
- **누적위험이 직선이다.** $H(t)=\lambda t$ 이므로 **$-\log\hat S(t)$ 를 $t$ 에 대해 그려 직선이면** 지수 가정이 그럴듯하다는 진단이 된다.
- **절단 자료의 최대가능도가 간단하다.** 사건 수를 총 노출시간으로 나눈 것이 $\hat\lambda$ 다.
