...
모수 모형은 **매끄러운 곡선과 외삽**을 준다.

- **카플란–마이어의 계단과 대조된다.** 분포를 가정한 대가로 매끄러운 $\hat S(t)$ 와 $\hat h(t)$ 를 얻고, **관측 범위 밖으로 외삽**할 수 있다.
...
- **외삽은 조심해야 한다.** 관측 기간 밖의 예측은 **전적으로 분포 가정에 의존**하므로, 5년 자료로 20년을 예측하는 일은 위험하다.

다음 절부터 **콕스 비례위험 모형**으로 넘어간다.
