...
??? success "풀이"
    ...
    **왜 그런가.** 바틀렛 통계량은 $\ln S_i^2$의 흩어짐을 잰다. $S_i^2$의 분산은
    ...
    $$
    \operatorname{Var}(S_i^2)=\frac{\sigma^4}{n}\left(\beta_2-1+\frac{2n}{n-1}\right)
    \quad\text{(}\beta_2\text{는 첨도)}
    $$

    로 **첨도에 직접 의존**한다. 바틀렛은 $\beta_2=3$(정규)을 전제로 기준분포를 잡으므로, 첨도가 다르면 **기준 자체가 틀린다.**
