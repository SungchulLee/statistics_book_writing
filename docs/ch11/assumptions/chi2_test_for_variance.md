...
??? success "풀이"
    ...
    **왜 중심극한정리가 구해 주지 않는가.** $S^2$의 분산이
    ...
    인데, 검정은 $\kappa_{\text{초과}}=0$을 전제로 $\chi^2(n-1)$을 쓴다. **$n$이 커져도 $\kappa_{\text{초과}}$ 항의 비중이 줄지 않는다.**

    $$
    \frac{\operatorname{Var}_{\text{실제}}(S^2)}{\operatorname{Var}_{\text{정규}}(S^2)}
    \ \xrightarrow[n\to\infty]{}\ 1+\frac{\kappa_{\text{초과}}}{2}
    $$

    **지수분포($\kappa_{\text{초과}}=6$)에서는 이 비가 4**다. 실제 $S^2$의 표준편차가 가정의 **두 배**이므로 오류율이 폭증한다.
    ...
    **대안 셋.**
    ...
    | 첨도 보정 | $\kappa$를 추정해 자유도를 조정(Box 1953) |
