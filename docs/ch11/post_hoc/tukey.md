...
연습문제 5의 **스튜던트화 범위 분포를 직접 만들어** `qsturng`과 대조하고, 임계값이 $k$에 따라 어떻게 자라는지 보라.
...
??? success "풀이"
    ...
    **본페로니가 생각만큼 나쁘지 않다.** 임계값이 5% 이내로 크다는 것은 **검정력 손실이 몇 퍼센트 수준**이라는 뜻이다(사후비교 개요 페이지 연습문제 6의 FWER 차이와 일관된다).
...
연습문제 6의 **불균형 설계**에서 튜키-크레이머가 실제로 FWER을 지키는지 재라.
...
??? success "풀이"
    ...
    **주의 — 이것은 등분산일 때의 이야기다.** 불균형 + **이분산**이면 튜키-크레이머가 무너진다(사후비교 개요 페이지 연습문제 9에서 FWER 0.362). **불균형 자체는 괜찮지만 이분산과 겹치면 위험하다.**
