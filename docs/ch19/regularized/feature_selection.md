...
변수선택은 **편향을 늘리지 않으면서 분산을 줄인다.**

- **무관한 변수는 편향을 줄이지 못하고 분산만 늘린다.** 그래서 빼는 것이 이득이며, 해석 가능성도 함께 좋아진다.
- **$L_1$ 정칙화가 선택과 추정을 동시에 한다.** 단계적 선택보다 안정적이고 계산도 효율적이다.
- **모형선택 일치성에는 조건이 붙는다.** 라쏘가 참 변수 집합을 정확히 골라내려면 무관한 변수와 관련 변수의 상관이 제한되어야 한다(비표현 조건). **그 조건이 깨지면 표본을 늘려도 잘못 고른다.**
- **선택 후 추론은 여전히 문제다.** 같은 자료로 고르고 검정하면 $p$ 값이 낙관적으로 편향된다.
- **안정성 선택이 보완책이다.** 부표본에서 반복해 자주 뽑히는 변수를 고르며, 17장의 재표집 아이디어다.

다음 절 **정칙화 로지스틱 회귀 실습**으로 넘어간다.
