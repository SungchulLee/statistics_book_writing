...
??? success "풀이"
    ...
    ```
    lambda=0.04  [ 0.91379 -0.59897  0.     ]
    lambda=0.06  [ 0.75432 -0.47237  0.     ]
    lambda=0.08  [ 0.61828 -0.36294  0.     ]
    lambda=0.10  [ 0.49857 -0.26526  0.     ]
    lambda=0.12  [ 0.39045 -0.17568  0.     ]
    lambda=0.14  [ 0.29051 -0.09154  0.     ]
    lambda=0.16  [ 0.19604 -0.01074  0.     ]
    ```
...
- **$p$ 가 $n$ 에 비해 크면 과적합한다.** 훈련 범주를 날카롭게 가르려고 계수가 커지고 예측확률이 $0$ 과 $1$ 로 몰린다.
- **완전분리에서는 최대가능도가 발산한다.** 두 범주를 완벽히 가르는 선형결합이 있으면 계수가 무한대로 가며, **정칙화가 그 문제를 원천적으로 없앤다.**
- **18장의 벌점이 그대로 쓰인다.** $L_2$ 는 계수를 축소하고 $L_1$ 은 변수를 선택하며, 엘라스틱넷이 둘을 섞는다.
- **`sklearn` 의 `C` 는 $\lambda$ 의 역수다.** $C$ 가 작을수록 벌점이 강하며, **기본값 $C=1$ 이 이미 정칙화를 켜 놓은 상태**라는 점을 놓치기 쉽다.
- **표준화가 필수다.** 벌점이 척도에 의존하기 때문이며, 절편은 벌점에서 제외한다.
