...
로지스틱 회귀의 가능도는 **베르누이 곱**이다.

$$
\ell(\boldsymbol\theta)=\sum_i\Bigl[y_i\log p_i+(1-y_i)\log(1-p_i)\Bigr]
$$

- **교차엔트로피 손실의 음수와 같다.** 통계학의 로그가능도 최대화와 기계학습의 손실 최소화가 **같은 계산**이며, 이름만 다르다.
- **닫힌 형태 해가 없다.** 지수함수 때문에 점수방정식이 비선형이라 수치 최적화가 필요하다.
- **로그가능도가 오목하다.** 그래서 지역 최적이 곧 전역 최적이며, 최적화가 안정적이다. 이것이 로지스틱 회귀의 큰 장점이다.
- **완전분리가 있으면 최대가 존재하지 않는다.** 어떤 선형결합이 두 부류를 완벽히 가르면 계수가 무한대로 발산하며, 정칙화가 그 처방이다.
- **6장의 최대가능도 이론이 그대로 적용된다.** 일치성·점근정규성·피셔 정보량 모두 여기서 쓰인다.

다음 절 **로지스틱 회귀 실습**으로 넘어간다.
