...
이 장은 확률론과 통계적 추론을 잇는다. 확률표본으로부터 계산된 통계량이 그 자체로 고유한 분포를 갖는 확률변수임을 보이는 것이 출발점이다. 네 가지 기본 표본분포(Normal, 스튜던트 $t$, 카이제곱, $F$)를 소개하고, 가장 흔히 쓰이는 통계량들의 표본분포를 유도하며, 표본크기가 표준오차를 통해 추정의 정밀도를 어떻게 좌우하는지 보인다.
...
- **정규분포 (Z)** --- 정규분포를 따르는 통계량을 표준화할 때 나타나는 표준정규분포이며, 중심극한정리를 통해 매우 다양한 추정량의 대표본 근사로 쓰인다.
- **스튜던트 t 분포** --- 알려진 모표준편차 대신 표본표준편차를 사용하여 정규모집단의 평균을 추정할 때 나타난다. 추가된 불확실성을 더 두꺼운 꼬리로 반영하며, 자유도가 커지면 정규분포로 수렴한다.
...
- **X-bar의 표본분포 (Bernoulli)** --- 베르누이 모집단에서 평균의 표본분포를 모의실험한다.
...
- **4장** (분포) --- Normal, Binomial, Poisson, 지수분포의 성질과 결합분포 및 독립성.
