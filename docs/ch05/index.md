...
$S^2$의 표본분포와 표준오차를 이론으로 정리하고, 네 모집단에서 모의실험한다. 카이제곱 결과가 정규모집단 전용이라는 사실이 여기서 드러난다. 어긋남의 배율 $(\kappa-1)/2$는 **표본크기를 아무리 키워도 그대로 남는다.**
...
$F$ 분포에 기댄 분산비 추론을 정리한 뒤 모의실험한다. 정규모집단에서는 정확하지만 비정규모집단에서는 명목 5% 검정의 실제 오류율이 27%, 49%까지 올라간다. **이 장에서 유일하게 표본을 키울수록 나빠지는 절차다.** 그래서 브라운–포사이드 검정 같은 강건한 대안이 왜 표준 자리를 차지했는지를 마지막에 다룬다.
