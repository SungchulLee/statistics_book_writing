...
모집단이 치우쳐 있으면서 표본까지 작으면 그냥 믿을 수 없다고 보는 편이 낫다. 이때는 정확검정이나 재표본추출로 옮겨 가야 한다. 이항 자료처럼 이산인 경우에는 별도의 잣대가 필요해서, 느슨한 기준 $np \geq 5$, $n(1-p) \geq 5$를 먼저 확인한다. 이 문턱이 보장하는 것은 분포의 모양까지이며, 구간이나 검정의 오류율까지 따질 때는 $10$을 쓴다.
