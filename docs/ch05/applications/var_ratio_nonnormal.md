...
| 모집단 | 첨도 $\kappa$ | 명목 5% 검정의 실제 오류율 | 명목 95% 구간의 포함률 |
|---|---|---|---|
| 정규 | 3.0 | 0.052 | 0.952 |
| 균등 | 1.8 | **0.005** | 0.994 |
| 지수 | 9.0 | **0.267** | 0.735 |
...
델타법으로 $\text{Var}(\log S^2) \approx \text{Var}(S^2)/\sigma^4$이고, 5.6절의 결과 $\text{Var}(S^2) \approx (\kappa-1)\sigma^4/n$을 넣으면

$$
\text{Var}(\log S_i^2) \approx \frac{\kappa - 1}{n_i}
$$

이다. 두 항이 독립이므로 더하면

$$
\text{Var}\!\left(\log \frac{S_1^2}{S_2^2}\right) \approx (\kappa-1)\left(\frac{1}{n_1} + \frac{1}{n_2}\right)
$$

가 된다. 정규 가정($\kappa = 3$)은 이 값을 $2\left(\frac{1}{n_1}+\frac{1}{n_2}\right)$로 믿는다. 따라서 **분산의 배율이 $\dfrac{\kappa-1}{2}$**이며, 5.6절에서 $S^2$ 하나에 대해 얻었던 것과 **정확히 같은 배율**이다.
...
$n$이 크면 $\log F$가 근사적으로 정규이므로 오류율을 손으로 계산할 수 있다. $F$ 검정은 $\log F$의 표준편차를 $\sqrt{2(1/n_1+1/n_2)}$로 믿고 $\pm 1.96$배만큼 기각역을 잡는데, 참 표준편차는 $\sqrt{(\kappa-1)(1/n_1+1/n_2)}$다. 따라서 기각역이 참 표준편차 단위로는 $\pm 1.96\sqrt{2/(\kappa-1)}$만큼만 뻗고
...
| 모집단 | $\kappa$ | $1.96\sqrt{2/(\kappa-1)}$ | 극한 오류율 |
|---|---|---|---|
| 균등 | 1.8 | 3.10 | 0.002 |
| 정규 | 3.0 | 1.96 | 0.050 |
| 지수 | 9.0 | 0.98 | 0.327 |
...
!!! note "주요 관찰"
    ...
    1. **어긋남의 크기가 첨도로 정해진다.** 배율 $(\kappa-1)/2$가 5.6절의 $S^2$ 하나짜리 결과와 같다. 비를 잡아도 새로운 원인이 생기는 것이 아니라 같은 원인이 두 번 들어간다.
    ...
    4. **극한값을 손으로 계산할 수 있다.** $2[1-\Phi(1.96\sqrt{2/(\kappa-1)})]$이며 모의실험과 맞는다. 자료의 첨도를 재 보면 $F$ 검정을 믿을 수 있는지 미리 판단할 수 있다.
...
??? success "풀이"
    지수분포의 첨도는 $\kappa = 9$이므로

    $$
    1.96\sqrt{\frac{2}{\kappa-1}} = 1.96\sqrt{\frac{2}{8}} = 1.96 \times 0.5 = 0.98
    $$
...
$\text{Var}(\log S^2) \approx (\kappa-1)/n$을 델타법으로 유도하라.
...
??? success "풀이"
    ...
    이다. 5.6절에서 $\text{Var}(S^2) = \frac1n\left(\kappa - \frac{n-3}{n-1}\right)\sigma^4 \approx \frac{(\kappa-1)\sigma^4}{n}$이므로
...
??? success "풀이"
    **절차.** 두 표본을 합쳐 표본첨도 $\hat\kappa$를 계산하고 극한 오류율 공식에 넣어 본다.

    $$
    \hat\alpha_{\text{실제}} = 2\left[1 - \Phi\!\left(1.96\sqrt{\frac{2}{\hat\kappa-1}}\right)\right]
    $$

    이 값이 0.05에서 크게 벗어나면 $F$ 검정을 쓰지 않는다. $\hat\kappa$가 4를 넘으면 이미 실제 오류율이 0.1에 가까워진다.

    **한계가 세 가지 있다.**

    첫째, **표본첨도가 매우 불안정하다.** 4차 적률의 추정이라 표본크기에 따라 크게 흔들린다. $n = 20$짜리 자료에서 $\hat\kappa$를 믿기 어렵다.
    ...
    셋째, 첨도가 유한하지 않을 수도 있다. $t_4$ 이하의 꼬리에서는 $\kappa$가 존재하지 않으므로 공식 자체가 무의미하다.
...
명목 신뢰수준 $1-\alpha$에서 분산비 구간의 극한 포함률 공식을 쓰고, 로그정규($\kappa = 113.9$)에서 95% 구간의 극한 포함률을 구하라. 예제 3의 0.520과 견주어라.

</div>

??? success "풀이"
    검정의 극한 오류율이 $2[1-\Phi(z_{1-\alpha/2}\sqrt{2/(\kappa-1)})]$이므로 포함률은 그 여사건이다.

    $$
    \text{극한 포함률} = 2\Phi\!\left(z_{1-\alpha/2}\sqrt{\frac{2}{\kappa-1}}\right) - 1
    $$

    로그정규에서 $\kappa = 113.9$이므로
...
- 비정규 모집단에서 $S_1^2/S_2^2$은 $F$ 분포를 따르지 않는다. 어긋남의 배율은 $\log$ 척도의 분산 비로 $(\kappa-1)/2$이며, 5.6절의 $S^2$ 하나짜리 결과와 같은 양이다.
- 방향은 첨도로 갈린다. 균등($\kappa = 1.8$)에서는 오류율이 0.005로 지나치게 보수적이고, 지수($\kappa=9$)에서는 0.27, 로그정규($\kappa=114$)에서는 0.49로 위험하다.
- **표본크기가 상황을 악화시킨다.** $n$을 키우면 오류율이 극한값 $2[1-\Phi(1.96\sqrt{2/(\kappa-1)})]$으로 올라간다. 이 장에서 유일하게 "표본이 많을수록 나빠지는" 절차다.
- 신뢰구간의 포함률도 함께 무너진다. 로그정규 자료에서 명목 95% 구간이 참값을 52%만 담는다.
- 표본첨도로 위험을 미리 재는 방법은 한계가 크다. 특히 표본첨도의 상한 때문에 **가장 위험한 경우를 가장 못 알아본다.**
- 다음 쪽에서 첨도에 의존하지 않는 대안을 본다.
