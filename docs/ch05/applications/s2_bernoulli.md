...
!!! tip "비율의 표준오차와 이어진다"
    ...
    의 분자가 바로 이 페이지의 항등식이다. $\hat p(1-\hat p) = \frac{n-1}{n}S^2$이므로, **비율의 표준오차는 곧 표본분산을 꽂아 넣은 것**이다. 5.3절의 "비율의 표본분포"에서 $\sigma^2$을 따로 추정하지 않고 $\hat p$에서 바로 계산했던 이유가 여기에 있다.
...
네 모집단을 나란히 놓으면 5.3절의 결론이 나온다. **$\bar X$의 표본분포는 모집단을 가리지 않지만 $S^2$의 표본분포는 모집단마다 다른 이야기를 한다.** 다음 절 **$\bar X$의 표준오차**로 넘어가 표본분포의 폭을 하나의 수로 요약한다.
