...
표집분포가 정규에 얼마나 가까운지를 히스토그램보다 예민하게 보여 주는 그림은 무엇인가? 그 그림에서 꼬리의 어긋남이 어떻게 나타나는지 설명하라.

</div>

??? success "풀이"
    **Q-Q 그림**이다. 표본분위수를 이론 정규분위수에 대해 그려, 정규분포라면 점들이 직선 위에 놓인다.

    **히스토그램보다 나은 이유.**

    - 히스토그램은 구간 폭에 따라 모양이 달라지지만 Q-Q 그림에는 그런 임의성이 없다.
    - 히스토그램은 꼬리에서 막대 높이가 0에 가까워 차이를 볼 수 없다. Q-Q 그림은 꼬리의 몇 안 되는 점을 **가로축 끝에 넓게 펼쳐** 보여 준다. 정규근사가 가장 잘 깨지는 곳이 꼬리이므로 이 성질이 결정적이다.
    - 참조선이 직선이라 눈이 어긋남을 잘 잡아낸다. 종 모양 곡선과의 어긋남을 눈대중하는 것보다 훨씬 쉽다.

    **어긋남의 양상.**

    | 양상 | 뜻 |
    |---|---|
    | 오른쪽 끝이 선 **위**로 휨 | 오른쪽 꼬리가 두껍다 |
    | 오른쪽 끝이 선 **아래**로 휨 | 오른쪽 꼬리가 얇다 |
    | 양끝이 모두 선 위/아래 (역 S자 / S자) | 양쪽 꼬리가 두껍다 / 얇다 |
    | 전체가 아래로 볼록 | 오른쪽으로 치우침 |

    지수 모집단에서 $n=5$인 표본평균의 Q-Q 그림을 그리면 오른쪽 끝이 선 위로 뚜렷이 휜다. 오른쪽 치우침이 남아 있다는 신호다. $n$을 키우면 점점 펴진다.

    **주의.** Q-Q 그림의 양끝은 표집변동이 크므로 점 한두 개가 벗어나는 것은 흔한 일이다. 정규분포에서 뽑은 표본으로 Q-Q 그림을 여러 장 그려 보면 "정상적인 어긋남"이 어느 정도인지 감을 잡을 수 있다. 신뢰띠를 함께 그려 주는 도구(R의 `qqPlot`)도 있다.

<div class="drillbox" markdown>

**연습문제 9.** <span class="diff med" title="중간"></span>
표집분포를 히스토그램으로 그릴 때 오해를 부를 수 있는 시각화 선택을 세 가지 들고, 각각 어떻게 고쳐야 하는지 적어라.

</div>

??? success "풀이"
    **(1) 구간 수를 잘못 고른다.** 구간이 너무 많으면 잡음이 구조처럼 보이고(봉우리가 여러 개인 것처럼), 너무 적으면 치우침이나 이봉성이 사라진다.

    → 스콧 규칙이나 프리드먼-디아코니스 규칙(`bins='auto'`)을 쓰고, **몇 가지 구간 수로 그려 보아 결론이 바뀌지 않는지 확인**한다. 커널밀도추정을 함께 얹으면 구간 선택의 임의성을 완화할 수 있다.

    **(2) 가로축 범위를 다르게 잡고 여러 그림을 나란히 놓는다.** $n=5$와 $n=50$의 표집분포를 각각 자동 범위로 그리면 **둘 다 비슷한 폭으로 보인다.** 표준오차가 $1/\sqrt{10}$로 줄었다는 사실이 그림에서 완전히 사라진다.

    → 본문 예제처럼 `sharex=True`로 가로축을 묶는다. 표집분포 비교에서 이것이 가장 중요한 장치다. 세로축도 `density=True`로 맞춰야 면적이 1로 통일된다.

    **(3) 세로축을 0에서 시작하지 않는다.** 막대그래프에서 세로축을 잘라내면 작은 차이가 극적으로 보인다. 히스토그램은 **면적으로 확률을 읽는 그림**이므로 축을 자르면 그 해석이 무너진다.

    → 히스토그램의 세로축은 반드시 0에서 시작한다(꺾은선 그래프에서는 자를 수도 있다).

    **그 밖에.** 표본이 적은데 부드러운 곡선을 얹으면 없는 정밀도를 주장하는 셈이고, 모의실험 반복 횟수 $B$를 밝히지 않으면 독자가 몬테카를로 오차를 가늠할 수 없다. **$n$(표본크기)과 $B$(반복 횟수)를 그림 제목에 함께 적는 습관**이 혼동을 막는다.

<div class="drillbox" markdown>

**연습문제 10.** <span class="diff med" title="중간"></span>
겉보기에 비슷한 세 히스토그램 — 표집분포, 부트스트랩 분포, 베이즈 사후분포 — 을 구별하라. 각각 무엇을 반복하고 무엇에 대한 불확실성을 나타내는가?

</div>

??? success "풀이"

    | | 무엇을 반복하는가 | 무엇의 분포인가 | 알아야 하는 것 |
    |---|---|---|---|
    | 표집분포 | 참 모집단에서 표본 뽑기 | 통계량 $\hat\theta$ | 모집단(현실에서는 모름) |
    | 부트스트랩 분포 | 관측된 표본에서 재표집 | $\hat\theta^*$ | 표본만 |
    | 사후분포 | (반복 없음) | 모수 $\theta$ | 사전분포 + 가능도 |

    **표집분포**는 이론적 대상이다. "같은 실험을 무한히 되풀이하면 $\hat\theta$가 어떻게 흩어지는가"를 말한다. 모집단을 알아야 그릴 수 있으므로 실제 분석에서는 **직접 볼 수 없고** 이론이나 모의실험으로 근사한다. 본문의 그림들이 모집단을 알고 있다는 전제 아래 그린 것이다.

    **부트스트랩 분포**는 표집분포의 실용적 대역이다. 모집단 대신 경험분포에서 재표집하며, 중심이 참 $\theta$가 아니라 **관측된 $\hat\theta$ 근처**에 놓인다는 점이 중요하다. 그래서 부트스트랩 분포의 중심을 $\theta$의 추정값으로 읽으면 안 되고, 그 **폭**을 표준오차로 쓰거나 분위수를 신뢰구간으로 쓴다.

    **사후분포**는 성격이 근본적으로 다르다. 앞의 둘은 $\theta$를 고정된 미지의 상수로 두고 **추정량의 변동**을 그리지만, 사후분포는 $\theta$ 자체를 확률변수로 보고 **자료를 본 뒤 $\theta$가 어디에 있을 법한가**를 그린다. 반복 표집이라는 개념이 아예 들어 있지 않다.

    **해석의 차이.** 95% 신뢰구간은 "이 절차를 반복하면 95%가 참값을 담는다"는 뜻이고, 95% 신용구간은 "참값이 이 구간에 있을 확률이 95%"라는 뜻이다. 후자가 사람들이 원하는 해석이지만 전자만이 빈도주의 신뢰구간이 보장하는 바다.

    **그럼에도 셋이 종종 비슷해 보인다.** 표본이 크고 사전분포가 평평하면 베른슈타인-폰 미제스 정리에 따라 사후분포가 $\hat\theta$를 중심으로 표집분포와 같은 폭의 정규분포에 다가가고, 부트스트랩 분포도 마찬가지다. **숫자가 거의 같아도 답하고 있는 질문은 다르다**는 점을 놓치지 않는 것이 중요하다.
