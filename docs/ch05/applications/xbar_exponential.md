...
이 페이지에서는 밑바탕 모집단이 지수분포를 따를 때 표본평균 $\bar{X}$의 표본분포를 살펴본다. 지수분포는 오른쪽으로 심하게 치우쳐 있어 중심극한정리를 시험하기에 아주 좋은 사례이다. 모집단이 치우쳐 있어도 표본크기가 커지면 $\bar{X}$의 분포가 근사적으로 정규분포가 된다. $n$이 작으면 표본분포에 치우침이 여전히 눈에 띈다.
...
이런 자료에서 위의 세 패널이 곧바로 실무의 문제가 된다.

- **모집단(위)**: 소득 분포를 보고 "평균 소득"을 말하면 대다수 사람의 형편을 대표하지 못한다. 평균이 중앙값보다 한참 오른쪽에 있기 때문이다.
- **표본 하나(가운데)**: 실제로 우리가 손에 쥐는 것은 이 점 몇 개뿐이다. 다섯 명을 뽑았는데 우연히 고소득자가 한 명 끼면 표본평균이 크게 튄다.
- **표집분포(아래)**: 그 튐의 크기를 재는 것이 표준오차이며, 치우친 모집단일수록 $n$이 작을 때 이 분포 자체가 치우쳐 있어 정규근사를 쓸 수 없다.

소득처럼 치우친 자료에서는 **로그를 씌워 대칭으로 만든 뒤 분석하는** 방법이 흔히 쓰인다. 4장에서 본 로그정규분포가 그 배경이며, 14장에서 변환을 본격적으로 다룬다.
