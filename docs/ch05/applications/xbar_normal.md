...
앞의 두 쪽에서는 내내 "언제쯤 정규에 가까워지는가"를 물었다. 균등 모집단에서는 $n = 5$면 충분했고 지수 모집단에서는 30으로도 모자랐다. 이 쪽에서는 그 물음 자체가 사라진다. 모집단이 이미 정규라면 표본평균은 **모든** $n$에서 정확히 정규이기 때문이다.

$n = 2$여도 그렇다. 표본이 둘뿐이어도 $\bar{X}$의 분포는 정규에 가까운 무엇이 아니라 정확히 정규다. 중심극한정리를 부를 일이 없고 "충분히 크면"이라는 단서를 달 일도 없다.

이 사실이 고전적 추론 전체를 떠받친다. $t$ 검정도, $t$ 신뢰구간도, 분산분석도 모두 모집단이 정규라는 가정 위에서 정확한 분포를 갖는다. 뒤집어 말하면 그 가정이 깨지는 순간 모든 것이 근사로 내려앉는다는 뜻이기도 하다.

## 모집단 모형

모집단이 표준정규분포를 따른다고 하자. 즉 $\mu = 0$, $\sigma^2 = 1$인 경우다.
...
확률표본 $X_1, X_2, \ldots, X_n \overset{\text{iid}}{\sim} N(\mu, \sigma^2)$에 대해 표본평균은 다음 분포를 갖는다.

$$
\bar{X} \sim N\!\left(\mu, \frac{\sigma^2}{n}\right)
$$

앞의 두 쪽에서는 같은 모양의 식에 "$n$이 크면 근사적으로"라는 단서가 붙어 있었다. 여기에는 없다. 독립인 정규확률변수의 선형결합은 그 자체가 다시 정규분포이고 $\bar{X} = \frac{1}{n}\sum_{i=1}^n X_i$가 바로 그런 선형결합이므로, 표본크기가 얼마든 $\bar{X}$는 정확히 정규분포를 따른다. 점근적 논증이 전혀 필요 없다.
...
앞의 두 쪽과 똑같은 그림을 이번에는 정규 모집단에서 그린다. 맨 위는 모집단, 가운데는 거기서 뽑은 **표본 하나**, 맨 아래는 그런 표본을 1만 번 뽑아 얻은 표본평균들의 분포다. 세 패널의 가로 눈금을 같게 묶어 두었으므로 퍼짐을 눈으로 직접 견줄 수 있다. 이번에는 맨 위와 맨 아래의 모양이 같고 폭만 다를 것이다.
...
맨 위의 모집단 분포는 종 모양이다. 정규분포이니 당연하다. 맨 아래의 표본분포도 종 모양이고 같은 평균 $\mu = 0$을 중심으로 한다. 달라진 것은 폭뿐이다. 표본분포는 모집단보다 $\sqrt{n}$배 **좁아**, $n = 5$에서 모집단의 $\sigma = 1$에 비해 표준오차가 $\sigma/\sqrt{5} \approx 0.447$이다.

균등이나 지수의 경우와 결정적으로 다른 점이 여기 있다. 그 두 쪽에서는 아래 패널이 종 모양에 **가까워진** 것이었고 $n$이 작을 때는 어긋남이 눈에 띄었다. 여기서는 어긋남이 없다. 표본분포의 정규성이 근사가 아니라 **정확**하다.

그러니 $n$을 바꿀 때 달라지는 것도 폭 하나뿐이다. 모양은 언제나 정규이고 표준편차만 $\sqrt n$에 반비례해 줄어든다.
...
말로 한 논증을 적률생성함수로 확인해 두자. 곱하고 치환하는 세 줄이면 끝난다.
...
모집단이 정규이면 $\bar X\sim N(\mu,\sigma^2/n)$이 모든 $n$에서 **정확**하다. 근사가 아니다. 이유는 정규분포가 덧셈에 대해 닫혀 있기 때문이며, 적률생성함수를 곱해 보면 한 줄로 나온다(3장). 중심극한정리가 "$n$이 커지면"이라고 말하는 것과 달리 여기에는 아무 조건이 없어서 $n=2$에서도 그대로 성립한다.

이 쪽에서 가져갈 것은 바로 그 무조건성이다. $t$ 검정도, $t$ 신뢰구간도, 분산분석도 모두 정규모집단 가정 위에서 정확한 분포를 갖는다. 고전적 추론이 이토록 깔끔한 것은 정규성이라는 가정 하나를 빌려 왔기 때문이고, 그 가정이 깨지면 모든 것이 근사로 내려앉는다.

정규분포가 빌려주는 것이 하나 더 있다. $\bar X$와 $S^2$이 **독립**이라는 성질인데, 이것 역시 정규분포에서만 성립한다. $t$ 통계량의 분자와 분모가 독립이 되는 근거가 여기이며, 다른 어떤 분포도 이 성질을 갖지 않는다.

다음 절 **베르누이 모집단**으로 넘어간다. 모집단이 0과 1뿐인 극단적인 경우이며, 표본평균이 곧 표본비율이 된다.
