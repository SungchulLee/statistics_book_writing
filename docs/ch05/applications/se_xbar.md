...
여론조사 기사 끝에는 늘 "오차범위 ±3.1%포인트"라는 말이 붙는다. 그 3.1은 어디서 오는가. 조사 대상을 네 배로 늘리면 왜 절반밖에 줄지 않는가. 답은 모두 한 공식에 들어 있고, 이 쪽은 그 공식 하나를 다룬다.

앞 쪽에서 통계량의 표준오차란 그 표본분포의 표준편차라고 했다. 표본평균의 경우 그 값이 $\sigma/\sqrt n$이라는 단순한 꼴이 되며, 이것이 모평균의 추정량으로서 $\bar X$가 얼마나 정밀한지를 말해 주는 유일한 수다. 아래에서 공식을 두 줄로 유도하고 모의실험으로 확인한 다음, 분모의 $\sqrt n$이 실무에서 무엇을 뜻하는지를 따진다.
...
이 한 공식 안에 앞 쪽에서 본 구별이 그대로 들어 있다. $\sigma$는 모집단에서 개별 관측값이 퍼진 정도이고, 그것을 $\sqrt n$으로 나눈 $\sigma/\sqrt{n}$는 표본을 되풀이해 뽑을 때 $\bar{X}$가 퍼지는 정도다. $n > 1$이기만 하면 뒤의 것이 앞의 것보다 언제나 작고, $n$이 커질수록 더 작아진다.

## 유도는 두 줄이다

공식이 어디서 오는지는 짧다. $\bar X$는 $X_i$들의 합을 $n$으로 나눈 것이므로 상수를 분산 밖으로 빼면 $1/n^2$이 나오고, 관측값이 독립이므로 분산들이 그냥 더해진다.
...
독립성이 쓰인 자리가 가운데 등식 하나뿐이라는 점을 기억해 두는 것이 좋다. 관측값들이 서로 상관되어 있으면 바로 그 자리에서 공분산 항이 살아나 공식이 무너지는데, 연습문제 6에서 그 경우를 따진다. 정규성은 어디에도 쓰이지 않았으므로 이 공식은 모집단 모양을 가리지 않는다.
...
그래서 실무에서는 표본크기를 먼저 정해 놓고 정밀도를 확인하는 것이 아니라, 필요한 정밀도를 먼저 정하고 거꾸로 $n$을 푼다. 연습문제 2가 바로 그 계산이다. 여론조사의 표본수, 임상시험의 피험자 수, 품질검사의 표본 개수가 모두 이렇게 정해진다.
...
$\mathrm{SE}(\bar X)=\sigma/\sqrt n$은 이 책에서 가장 자주 쓰이는 공식이다. 유도는 두 줄로 끝난다. $\mathrm{Var}(\bar X)=\frac1{n^2}\sum\mathrm{Var}(X_i)=\frac{\sigma^2}{n}$을 구하고 제곱근을 취하면 된다. 독립성이 필요한 곳은 가운데 등식 하나뿐이고 정규성은 어디에도 쓰이지 않았으므로, 이 공식은 분산이 유한한 어떤 모집단에서도 그대로 성립한다.

실제로 쓸 때는 $\sigma$를 모르므로 $s$로 바꿔 $\widehat{\mathrm{SE}}=s/\sqrt n$을 쓴다. 그 대체가 $t$ 분포를 부르며, 표본이 작을 때는 대가가 작지 않다(3장 슬러츠키 문서). 그리고 줄어드는 것이 무엇인지도 잊지 말아야 한다. 작아지는 것은 추정값의 흔들림이지 자료의 흩어짐이 아니다. $\sigma$는 모집단의 성질이라 $n$과 아무 상관이 없다.

이 쪽에서 가져갈 것 하나를 고르라면 분모의 $\sqrt n$이다. **오차를 절반으로 줄이려면 표본을 네 배, 10분의 1로 줄이려면 백 배 늘려야 한다.** 표본크기 계산이 전부 이 관계에서 나오며, 정밀도가 곧 비용인 이유도 여기에 있다.

다만 이 모든 이야기는 독립을 전제로 한다. 독립이 아니면 공식이 무너진다. 3장에서 보았듯 AR(1) 자료에서는 유효표본크기가 $n(1-\phi)/(1+\phi)$로 줄어들어, $\phi=0.9$면 관측 만 개가 독립 오백 개 값어치밖에 되지 않는다.
