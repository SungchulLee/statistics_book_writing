...
$$
\mu_4 = E[(X-\mu)^4] = \int_0^1 \left(x - \tfrac12\right)^4 dx = \frac{1}{80}, \qquad \kappa = \frac{\mu_4}{\sigma^4} = \frac{144}{80} = 1.8
$$
...
$$
\frac{\text{Var}(S^2)}{2\sigma^4/(n-1)} = \frac{n-1}{2n}\left(\kappa - \frac{n-3}{n-1}\right) \;\xrightarrow{\;n \to \infty\;}\; \frac{\kappa - 1}{2}
$$

이다. 균등분포는 $\kappa = 1.8$이므로 **배율이 $0.4$**다.

!!! danger "이 어긋남은 표본을 키워도 사라지지 않는다"
    극한이 $(\kappa-1)/2$로 **$n$에 의존하지 않는다.** 정규모집단이면 $\kappa = 3$이라 배율이 정확히 1이 되지만, 균등모집단에서는 $n$이 아무리 커져도 $S^2$의 분산이 카이제곱 예측의 40%에 머문다. 표준편차로는 $\sqrt{0.4} = 0.63$배다.
...
!!! note "주요 관찰"
    ...
    3. **$n$을 키워도 고쳐지지 않는다.** 배율 $(\kappa-1)/2 = 0.4$는 표본크기와 무관하다.
...
$X \sim \text{Uniform}(0,1)$의 4차 중심적률을 직접 적분해 $\mu_4 = 1/80$임을 보이고 첨도 $\kappa = 1.8$을 확인하라. $\text{Uniform}(a,b)$에서도 같은 값인가?
...
??? success "풀이"
    ...
    $$
    \kappa = \frac{1/80}{1/144} = \frac{144}{80} = 1.8
    $$
...
$\text{Var}(S^2)$과 카이제곱이 예측하는 값의 비가 $n \to \infty$에서 $(\kappa-1)/2$로 감을 보여라. 정규모집단에서 이 값이 1이 되는지 확인하라.
...
??? success "풀이"
    ...
    $n \to \infty$이면 $\frac{n-1}{2n} \to \frac12$이고 $\frac{n-3}{n-1} \to 1$이므로 극한은 $\frac{\kappa-1}{2}$이다. $\square$

    정규모집단은 $\kappa = 3$이므로 $(3-1)/2 = 1$이다. 두 식이 일치한다. 사실 정규분포에서는 극한이 아니라 **모든 $n$에서 정확히** 1이다. $\kappa = 3$을 넣으면
...
??? success "풀이"
    ...
    **좋기만 한 것은 아니다.** 신뢰구간과 검정은 동전의 양면이므로, 구간이 넓다는 것은 대응하는 검정의 **유의수준이 명목보다 낮다**는 뜻이다. 즉 실제로 다른 분산을 가진 모집단을 놓고도 기각하지 못한다. **검정력을 잃는다.**
    ...
    | 어긋나는 방향 | 포함률 | 위험 |
    |---|---|---|
    | 가벼운 꼬리($\kappa < 3$) | 0.95보다 높다 | 검정력 손실(보수적) |
    | 무거운 꼬리($\kappa > 3$) | 0.95보다 낮다 | 잘못된 유의성(위험) |
...
??? success "풀이"
    ...
    $$
    \sqrt n\,(S^2 - \sigma^2) \;\xrightarrow{d}\; N\!\left(0,\ \mu_4 - \sigma^4\right) = N\!\left(0,\ (\kappa-1)\sigma^4\right)
    $$

    균등분포에서는 $(\kappa - 1)\sigma^4 = 0.8/144 = 0.00556$이므로 $n = 100$에서
    ...
    **그림에서 히스토그램이 이미 좌우 대칭인 종 모양**인 것이 이 수렴을 보여 준다. 반면 빨간 카이제곱 곡선도 $n$이 크면 종 모양이 되지만 **폭이 다르다**($\text{SD} = 0.0118$로 1.58배). 즉 두 분포는 모양에서 가까워지고 폭에서 갈라진다.
    ...
    $S^2$의 극한이 정규라는 사실은 실무적 대안을 준다. 첨도를 자료에서 추정해 $\widehat{\text{Var}}(S^2) = (\hat\kappa - 1)s^4/n$으로 두고 정규 기반 구간을 만들면 모집단 모양에 덜 민감하다. 다만 $\hat\kappa$ 자체가 불안정하므로 실무에서는 부트스트랩을 더 흔히 쓴다.
...
- 배율 $(\kappa-1)/2$는 **표본크기와 무관**하다. $n$을 키워도 어긋남이 사라지지 않는다는 점이 $\bar X$와 결정적으로 다르다.
