...
합동 $t$ 검정은 **회귀분석의 특수한 경우**다. 집단 표시 더미변수 $D_i \in \{0,1\}$에 대한 단순회귀 $Y = \beta_0 + \beta_1 D + \varepsilon$에서 $\hat\beta_1$과 그 $t$ 통계량이 무엇이 되는지 보여라.

</div>

??? success "풀이"
    **최소제곱 해를 직접 구하면 된다.** $D$가 $0$ 또는 $1$뿐이므로

    $$
    \hat\beta_0 = \bar Y_{D=0} = \bar X_1,
    \qquad
    \hat\beta_0 + \hat\beta_1 = \bar Y_{D=1} = \bar X_2
    $$

    이다. 즉

    $$
    \hat\beta_1 = \bar X_2 - \bar X_1
    $$

    로 **기울기가 곧 두 평균의 차**다. 절편은 기준집단의 평균이다.

    **잔차분산도 일치한다.** 회귀의 잔차는 각 관측에서 자기 집단 평균을 뺀 것이므로

    $$
    \hat\sigma^2 = \frac{\sum_i(y_i-\hat y_i)^2}{n-2}
    = \frac{\sum_{\text{1군}}(x-\bar X_1)^2 + \sum_{\text{2군}}(x-\bar X_2)^2}{n_1+n_2-2}
    = S_p^2
    $$

    로 **합동분산 그 자체**다. 자유도 $n-2$도 합동 $t$의 $n_1+n_2-2$와 같다.

    **따라서 $t$ 통계량도 같다.** 회귀에서

    $$
    \operatorname{SE}(\hat\beta_1) = \hat\sigma\sqrt{\frac{1}{n_1}+\frac{1}{n_2}}
    $$

    이므로 $t = \hat\beta_1/\operatorname{SE}(\hat\beta_1)$이 합동 $t$와 완전히 일치한다. $\square$
    ...
    rng = np.random.default_rng(1)
    x, y = rng.normal(0, 1, 12), rng.normal(0.5, 1, 15)
    t, p = stats.ttest_ind(y, x, equal_var=True)

    Y = np.concatenate([x, y])
    D = np.concatenate([np.zeros(len(x)), np.ones(len(y))])
    fit = sm.OLS(Y, sm.add_constant(D)).fit()

    print(f"  합동 t : t={t:.6f}  p={p:.6f}")
    print(f"  회귀   : t={fit.tvalues[1]:.6f}  p={fit.pvalues[1]:.6f}")
    print(f"  beta1={fit.params[1]:.6f}   평균 차={y.mean()-x.mean():.6f}")
    ```

    출력:

    ```
      합동 t : t=-0.170781  p=0.865771
      회귀   : t=-0.170781  p=0.865771
      beta1=-0.056424   평균 차=-0.056424
    ```

    **소수점 여섯 자리까지 같다.** 기울기 추정값도 두 평균의 차와 정확히 일치한다.

    **왜 이것이 중요한가.**

    - **등분산 가정의 정체가 드러난다.** 회귀는 오차분산이 설명변수와 무관하다고 가정하는데(등분산성), 집단이 둘뿐인 경우 그것이 곧 $\sigma_1^2 = \sigma_2^2$다. 13장에서 다룰 이분산 문제가 여기서는 웰치 문제와 같은 것이다.
    - **확장이 자유로워진다.** 집단이 셋이면 더미를 둘 쓰고, 공변량을 넣으면 공분산분석(ANCOVA)이 되며, 상호작용을 넣으면 집단별 기울기가 달라진다. **$t$ 검정은 출발점일 뿐**이다.
    - **같은 자료를 한 틀에서 다룬다.** 13장의 회귀, 11장의 분산분석, 이 쪽의 $t$ 검정이 모두 **선형모형**의 다른 얼굴이다.

<div class="drillbox" markdown>

**연습문제 9.** <span class="diff med" title="중간"></span>
집단이 둘일 때 일원배치 분산분석의 $F$ 통계량과 합동 $t$ 통계량 사이에 $F = t^2$이 성립한다. 이를 보이고, 두 검정의 $p$값이 같음을 확인하라. 집단이 셋 이상이면 왜 이 대응이 깨지는가?

</div>

??? success "풀이"
    **대수적으로 확인한다.** $k=2$, $n_1+n_2=N$일 때 집단 간 제곱합은

    $$
    \text{SSB} = n_1(\bar X_1-\bar X)^2 + n_2(\bar X_2-\bar X)^2
    = \frac{n_1n_2}{N}(\bar X_1-\bar X_2)^2
    $$

    이고 자유도가 $1$이다. 집단 내 제곱합은 $\text{SSW} = (N-2)S_p^2$으로 자유도 $N-2$다. 따라서

    $$
    F = \frac{\text{SSB}/1}{\text{SSW}/(N-2)}
    = \frac{\frac{n_1n_2}{N}(\bar X_1-\bar X_2)^2}{S_p^2}
    = \frac{(\bar X_1-\bar X_2)^2}{S_p^2\left(\frac1{n_1}+\frac1{n_2}\right)} = t^2
    $$

    이다($\frac{n_1n_2}{N}$의 역수가 $\frac1{n_1}+\frac1{n_2}$임을 쓴다). $\square$

    **분포로도 맞는다.** $t \sim t_{N-2}$이면 $t^2 \sim F_{1,N-2}$이다. 5.3절 $F$ 분포에서 본 관계이며, $p$값이 같은 것이 이 때문이다.
    ...
    rng = np.random.default_rng(1)
    x, y = rng.normal(0, 1, 12), rng.normal(0.5, 1, 12)
    t, pt = stats.ttest_ind(x, y)
    F, pF = stats.f_oneway(x, y)
    print(f"  t={t:.6f}  t^2={t**2:.6f}")
    print(f"  F={F:.6f}")
    print(f"  p(t)={pt:.6f}  p(F)={pF:.6f}")
    ```

    출력:

    ```
      t=-1.012284  t^2=1.024719
      F=1.024719
      p(t)=0.322407  p(F)=0.322407
    ```

    **소수점 여섯 자리까지 같다.**

    **집단이 셋 이상이면 대응이 깨진다.** 이유는 **자유도**다. $k$개 집단이면 $F$의 분자 자유도가 $k-1$이 되어 $F_{k-1,N-k}$인데, $t^2$은 언제나 $F_{1,\cdot}$이다. $k=2$일 때만 $k-1=1$이라 일치한다.

    **개념적으로는 "질문이 하나인가 여럿인가"의 차이다.** 집단이 둘이면 물을 수 있는 차이가 하나뿐이지만, 셋이면 $\mu_1-\mu_2$, $\mu_1-\mu_3$, $\mu_2-\mu_3$의 세 방향이 있다. $F$는 그 **모두를 한꺼번에** 검정하므로 하나의 $t$로 환원되지 않는다.

    **그래서 $F$가 유의할 때 "어느 쌍이 다른가"를 따로 물어야 하고**, 그것이 11장의 사후비교이며 연습문제 8(대비)이 그 도구다. 다중성 보정이 필요한 것도 이 지점부터다.

<div class="drillbox" markdown>

**연습문제 10.** <span class="diff hard" title="어려움"></span>
표준화 효과크기 **코헨의 $d = (\bar X_2-\bar X_1)/S_p$**는 $\delta = (\mu_2-\mu_1)/\sigma$의 불편추정량이 아니다. 편향의 방향과 크기를 확인하고, **헤지스의 $g$** 보정을 적용하라.

</div>

??? success "풀이"
    **편향의 원인은 분모다.** $S_p$가 $\sigma$를 과소추정하므로($E[S_p] = c_4\sigma < \sigma$, 5.3절 카이제곱 연습문제 10) 그 역수인 $1/S_p$는 과대추정되고, 결과적으로 $d$가 $\delta$보다 크게 나온다.

    보정계수는

    $$
    J = 1 - \frac{3}{4\,\text{df}-1}, \qquad \text{df} = n_1+n_2-2
    $$

    이고 $g = J\cdot d$가 근사적으로 불편이다.
    ...
    d_true = 0.5
    print(f"{'집단당 n':>10}{'E[d]':>10}{'참 d':>8}{'J':>10}{'E[g]':>10}")
    for n in (5, 10, 20, 50):
        X = rng.normal(0, 1, (60_000, n))
        Y = rng.normal(d_true, 1, (60_000, n))
        sp = np.sqrt(((n-1)*X.var(1, ddof=1) + (n-1)*Y.var(1, ddof=1)) / (2*n-2))
        d = (Y.mean(1) - X.mean(1)) / sp
        J = 1 - 3 / (4*(2*n-2) - 1)
        print(f"{n:>10}{d.mean():>10.4f}{d_true:>8.1f}{J:>10.4f}{(d*J).mean():>10.4f}")
    ```
    ...
    ```
        집단당 n      E[d]     참 d         J      E[g]
             5    0.5550     0.5    0.9032    0.5013
            10    0.5232     0.5    0.9577    0.5011
            20    0.5099     0.5    0.9801    0.4998
            50    0.5037     0.5    0.9923    0.4999
    ```

    **$n=5$에서 $d$가 $11\%$ 위로 편향되어 있다.** 보정 후 $g = 0.5013$으로 거의 정확해진다. $n=50$이면 편향이 $0.7\%$로 줄어 보정이 거의 필요 없다.

    **왜 이것이 실무에서 문제가 되는가.**

    - **메타분석.** 여러 연구의 효과크기를 합칠 때 표본이 작은 연구가 체계적으로 큰 $d$를 보고하면 통합 추정값이 부풀려진다. 메타분석에서 헤지스의 $g$를 표준으로 쓰는 이유다.
    - **표본크기 설계.** 연습문제 9에서 예비연구의 $d$로 설계하면 표본을 과소 산정한다고 했는데, 그 한 원인이 이 편향이다. 예비연구는 $n$이 작으므로 편향이 크다.
    - **출판 편향과 겹친다.** 유의한 결과만 보고되면 $d$가 한 번 더 위로 치우친다(1장). **두 편향이 같은 방향**이라 효과크기 문헌이 전반적으로 과대평가되어 있을 수 있다.

    **$J$의 모양도 읽어 두자.** $\text{df}$가 크면 $J \to 1$이므로 큰 표본에서는 $d$와 $g$가 사실상 같다. **보정이 중요한 것은 $\text{df} < 20$, 즉 집단당 $10$명 남짓일 때**다.
