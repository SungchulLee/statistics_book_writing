...
이 페이지는 **모든 가정이 맞는 경우**를 본다. 두 모집단이 정규이고, 분산이 같고, 표본크기도 같다. 이 자리에서는 근사가 하나도 필요하지 않다.
...
정리 1을 $\mu_1 - \mu_2$에 대해 풀면 신뢰구간이 된다.

$$
(\bar X_1 - \bar X_2) \pm t_{\alpha/2,\, n_1+n_2-2}\, S_p\sqrt{\frac{1}{n_1}+\frac{1}{n_2}}
$$

$H_0: \mu_1 = \mu_2$ 검정은 $\mu_1-\mu_2=0$을 넣은 $T$를 $t_{n_1+n_2-2}$와 견주는 것이고, 이는 위 구간이 0을 담는지 보는 것과 완전히 같다.
...
$n_1 = n_2 = 15$, $s_1^2 = 1.10$, $s_2^2 = 0.86$, $\bar x_1 - \bar x_2 = -0.42$인 자료에서 $S_p^2$, 표준오차, 95% 신뢰구간을 구하고 $H_0: \mu_1 = \mu_2$를 5% 수준에서 검정하라.
...
??? success "풀이"
    ...
    $$
    \text{SE} = s_p\sqrt{\frac{1}{15}+\frac{1}{15}} = 0.9899 \times 0.3651 = 0.3615
    $$

    이고 $t_{0.975,\,28} = 2.0484$이므로

    $$
    -0.42 \pm 2.0484 \times 0.3615 = -0.42 \pm 0.7405 = (-1.160,\ 0.320)
    $$

    이다. 검정통계량은

    $$
    t = \frac{-0.42}{0.3615} = -1.162
    $$

    이고 $|t| < 2.0484$이므로 **기각하지 못한다.** 신뢰구간이 0을 담고 있는 것과 같은 결론이다.

    참 차이가 $-0.5$인 모집단에서 나온 자료인데도 기각하지 못했다. 예제 3에서 본 검정력 0.26이 바로 이 상황이며, 집단당 15개로는 이 크기의 차이를 놓치는 것이 오히려 흔한 일이다.
...
효과크기 $(\mu_1-\mu_2)/\sigma = 0.5$를 양측 5%에서 검정력 80%로 검출하려면 집단당 몇 개가 필요한가? 정규근사로 먼저 구한 뒤 비중심 $t$ 분포로 보정하고, 두 값이 다른 이유를 설명하라.
