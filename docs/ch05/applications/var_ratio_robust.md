...
이로써 5.3절부터 이어진 여섯 통계량을 모두 보았다. 마지막으로 한 문장으로 요약하면, **중심극한정리는 1차 적률에만 선물을 주고 2차 적률에는 주지 않는다.** $\bar X$, $\hat p$, $\bar X_1 - \bar X_2$, $\hat p_1 - \hat p_2$는 표본을 키우면 정규성이 필요 없어지지만, $S^2$과 $S_1^2/S_2^2$은 표본을 키워도 정규성에서 벗어나지 못한다.
