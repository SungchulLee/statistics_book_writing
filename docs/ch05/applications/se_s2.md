...
균등분포 $\text{Uniform}(0, 1)$에서 다섯 개를 뽑아 표본분산을 계산하는 일을 되풀이한다고 하자. 그 값들의 평균을 내면 참 분산 $1/12 \approx 0.0833$ 가까이 나온다. 그러나 한 번 한 번의 값은 그 근처에 얌전히 머물지 않는다. 아래에서 계산해 보면 표준오차가 0.0425여서, 값이 0.04 아래로 내려가거나 0.13 위로 올라가는 일이 드물지 않다.

표본평균 $\bar{X}$가 표본 간 변동성을 재는 표준오차를 갖듯이 표본분산 $S^2$도 표준오차를 갖는다. 모분산 $\sigma^2$을 추정하거나 그에 관해 추론할 때 $S^2$의 정밀도를 이해하는 것이 중요하다.

다만 공식의 재료가 다르다는 점이 이 쪽의 핵심이다. $\bar{X}$의 표준오차 $\sigma/\sqrt{n}$에는 모집단의 2차 적률만 들어가지만, $S^2$의 표준오차에는 **4차 적률**이 들어간다. 아래에서 그 공식을 유도하고, 균등 모집단에서 모의실험으로 추정하며, 그림으로 나타낸다.

## 표준오차의 정의와 일반 공식

표준오차란 통계량의 표집분포의 표준편차이므로 정의 자체는 간단하다. 어려운 것은 그 값을 무엇으로 적을 수 있느냐다.
...
마지막 식이 이 쪽에서 가장 중요한 한 줄이다. $\mu_4$는 모집단의 꼬리가 얼마나 두꺼운지를 재는 양이고, 그것이 $S^2$의 흔들림을 정한다. **$S^2$이 얼마나 정밀한지는 모집단의 분산이 아니라 모집단의 꼬리가 결정한다.** 정규모집단은 $\mu_4 = 3\sigma^4$이라는 특별한 관계를 만족하는 경우일 뿐이며, 그 값을 넣었을 때 앞의 닫힌 형태가 나온다.

## 균등분포에 넣어 보면

일반 공식이 실제로 어떻게 쓰이는지 한 번 따라가 보자. $X \sim \text{Uniform}(0, 1)$의 분산과 4차 중심적률은 다음과 같다.

$$
\sigma^2 = \frac{1}{12}, \qquad \mu_4 = \frac{1}{80}
$$

$n = 5$인 표본을 생각하고 위의 두 값을 일반 공식에 넣으면 분산이 나온다.
...
평균이 0.0833인데 표준오차가 0.0425다. 다섯 개짜리 표본으로 분산을 재면 값이 절반으로 줄거나 1.5배로 커지는 일이 드물지 않다는 뜻이다.
...
먼저 숫자가 맞는지 본다. 1만 개 $S^2$ 값의 평균은 0.0838로 이론적 평균 $E[S^2] = \sigma^2 = 1/12 \approx 0.0833$에 잘 맞고, 그 표준편차는 0.0425로 앞에서 손으로 계산한 이론적 표준오차와 소수 넷째 자리까지 일치한다. $S^2$이 불편추정량이라는 것과 우리가 쓴 분산 공식이 옳다는 것이 한꺼번에 확인된 셈이다.
...
임이 알려져 있다. $n=2, 5, 10, 25, 50$에서 편향의 크기를 구하고, $S^2$은 불편인데 $S$는 왜 아닌지 설명하라.
...
그러나 이 식은 특별한 경우일 뿐이다. 일반적인 모집단에서는 $\mathrm{Var}(S^2)\approx(\mu_4-\sigma^4)/n$ 이어서 **$\bar X$ 와 달리 4차적률이 필요하고**, 정규분포가 만족하는 $\mu_4=3\sigma^4$ 을 넣었을 때에야 위 공식이 나온다.

그러므로 **초과첨도가 곧 벌점이다.** $\gamma_2$ 가 양수면 실제 표준오차가 정규 공식보다 크고, 그 차이가 $\sqrt{(\gamma_2+2)/2}$ 배다. $\gamma_2=6$ 이면 **두 배**이며, 이 배율은 표본을 키운다고 줄어들지 않는다.

정밀도의 값도 만만치 않다. **상대 정밀도가 $\sqrt{2/(n-1)}$** 이므로 분산을 $10\%$ 정밀도로 추정하려면 $n\approx200$ 이 필요하다. 같은 정밀도로 평균을 추정하는 것보다 훨씬 많은 자료가 든다. 분산·상관·첨도로 갈수록 필요한 적률의 차수가 올라가고 추정이 어려워지는데, **그 순서가 곧 신뢰도의 순서다.**

다음 쪽부터는 네 모집단에서 $S^2$ 의 표본분포를 직접 모의실험한다. $\bar X$ 쪽과 달리 **카이제곱 결과가 정규모집단에서만 성립**하므로, 모집단을 바꾸면 결론도 바뀐다.

## 그래서 이 표준오차를 어디에 쓰는가

$\operatorname{SE}(S^2)$이 4차 적률을 요구한다는 사실이 이 쪽의 요점이다. $\bar X$의 표준오차는 $\sigma$ 하나만 알면 되었는데, $S^2$의 표준오차는 모집단의 첨도까지 알아야 한다. 그리고 첨도는 표본에서 추정하기가 매우 나쁜 양이다.

이 값이 실제로 쓰이는 자리는 $\sigma^2$의 [신뢰구간](../../ch08/one_sample_intervals/ci_sigma2.md)과 [분산 검정](../../ch09/one_sample_tests/test_variance.md)이며, 각각 8.2절과 9.2절에서 다룬다. 그때 쓰는 카이제곱 기반 구간은 정규성을 전제로 $\beta_2 = 3$을 못박아 둔 것이므로, 자료의 첨도가 3에서 멀면 구간의 폭 자체가 틀린다.
