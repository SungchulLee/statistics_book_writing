...
$\sigma_1^2$과 $\sigma_2^2$을 안다면 차의 분산을 그대로 쓸 수 있고, 표준화한 양이 정확히 표준정규를 따른다.

$$
Z = \frac{(\bar{X}_1 - \bar{X}_2) - (\mu_1 - \mu_2)}{\sqrt{\frac{\sigma_1^2}{n_1} + \frac{\sigma_2^2}{n_2}}} \sim N(0, 1)
$$

이 식을 $\mu_1 - \mu_2$에 대해 풀면 신뢰구간이 된다. 추정값을 가운데 두고 임계값과 표준오차의 곱만큼 양쪽으로 벌리는 형태다.
...
신뢰구간도 $\sigma_i$ 자리에 $S_i$가 들어간 것 말고는 경우 A와 같다.
...
신뢰구간은 임계값을 $z$에서 $t$로 바꾸고 표준오차에 $S_p^2$을 쓴 것이다.
...
??? success "풀이"
    ...
    Welch 자유도: $\nu = (\mathrm{SE}^2)^2/[(s_A^2/n_A)^2/(n_A - 1) + (s_B^2/n_B)^2/(n_B - 1)] \approx (4.88)^2/[(2)^2/49 + (2.88)^2/49] = 23.81/0.2509 \approx 94.9$. 95로 반올림한다.
