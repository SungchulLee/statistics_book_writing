...
!!! tip "실무에서는 Welch를 기본으로 둔다"
    위 표는 "조건을 알 때 무엇을 쓰는가"를 정리한 것이다. 그런데 현실에서는 $\sigma_1^2 = \sigma_2^2$인지를 **모른다.** 그렇다고 등분산 검정을 먼저 하고 그 결과로 합동 $t$와 Welch $t$를 고르는 2단계 절차를 쓰면 안 된다. 자료를 보고 방법을 고르는 순간 전체 유의수준이 통제되지 않고, 게다가 등분산 검정 자체의 검정력이 낮아 그 판정을 믿을 수도 없다(5.9절).
