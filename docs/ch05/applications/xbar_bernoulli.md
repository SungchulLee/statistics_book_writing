...
남는 물음은 "충분히 크면"이 얼마를 뜻하느냐다. $p$가 0.5 근처면 모집단이 대칭이라 수렴이 빠르고, $p$가 0이나 1 가까이 가면 한쪽으로 심하게 치우쳐 훨씬 느리다. 그래서 표본크기 $n$만 보는 대신 $np$와 $n(1-p)$, 곧 **기대되는 성공 횟수와 실패 횟수**를 함께 보는 경험 법칙을 쓴다.
...
!!! note "주요 관찰"
    ...
    네 패널 어디서나 모의실험한 $\hat{p}$의 히스토그램이 붉은 정규곡선과 잘 겹친다. $n = 100$이면 $p = 0.4,\, 0.5,\, 0.6,\, 0.7$ 모두에서 $np$와 $n(1-p)$가 30 이상이므로 경험 법칙이 넉넉히 충족되고, 기대한 대로 근사가 잘 작동한 것이다.
...
수렴의 속도는 $p$ 가 얼마나 극단적인지에 달려 있다. $np\ge5$ 이고 $n(1-p)\ge5$ 라는 관례가 표본크기 대신 기대 성공·실패 횟수를 보는 이유가 이것이며, 3장에서 재어 보니 $p=0.05$ 에서는 $n$ 이 만 단위로 필요했다. 게다가 이산분포를 연속분포로 흉내 내는 일이라 격자 구조 때문에 왜도만으로는 설명되지 않는 오차가 더해지고, **연속성 보정**이 필요한 까닭도 그것이다.
