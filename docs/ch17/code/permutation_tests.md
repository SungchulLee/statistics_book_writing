...
??? success "연습문제 1 풀이"
    ...
    **이 성질이 왜 중요한가.** $p$값이 균등분포라는 것은 제1종 오류 통제와 **동치**이다.
    ...
    이 진단은 다른 상황으로 그대로 확장된다. [기초](../permutation/foundations.md) 연습문제 1에서 이분산·불균형 표본의 순열검정이 이 균등성을 잃는 것을 보았다.
...
??? success "연습문제 2 풀이"
    ...
    **$p$값의 정의에 $+1$ 보정이 반드시 들어가야 한다.**
    ...
    !!! warning "$+1$을 빼면 증명이 무너진다"
        ...
        [대응 순열검정](../permutation/paired.md) 연습문제 3에서 $B = 199$, $\alpha = 0.05$일 때 보정 없는 정의의 제1종 오류율이 $0.0512$로 명목값을 넘고, 보정하면 $0.0458$로 내려가는 것을 수치로 확인했다.
...
??? success "연습문제 4 풀이"
    ...
    **정확한 이론값과 비교할 수 있다.** 순열분포는 초기하분포에서 정확히 유도된다([기초](../permutation/foundations.md) 연습문제 4). 처치군의 전환 수를 $X$라 하면 $X \sim \text{Hypergeometric}(46327, 382, 22588)$이고
