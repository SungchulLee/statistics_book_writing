...
**연습문제 1.** <span class="diff hard" title="어려움"></span> 단순선형회귀에서 LOOCV에는 지름길 공식이 있다. LOOCV 평균제곱오차가 다음과 같음을 보여라.
...
**연습문제 2.** <span class="diff med" title="중간"></span> 모의실험 연구를 수행하라. 같은 모형에서 $100$개의 자료를 생성한다. 각 자료에서 검증집합, 5-겹, 10-겹, LOOCV로 최적 다항 차수를 선택하고, 각 방법별로 선택된 차수의 분포를 보고하라. 어느 방법이 가장 안정적인가?
...
**연습문제 3.** <span class="diff hard" title="어려움"></span> LOOCV 추정량의 기댓값이 참 기대 검정 MSE에 대해 근사적으로 불편임을 증명하라. 구체적으로 $E[\text{CV}_{(n)}] \approx E[\text{MSE}_{\text{test}}]$임을 보여라. 여기서 검정 MSE는 같은 분포에서 나온 독립 관측에서 평가된다.
...
**연습문제 4.** <span class="diff med" title="중간"></span> 반복 $k$-겹 교차검증을 구현하라. 서로 다른 무작위 섞기로 10-겹 교차검증을 다섯 번 실행하고 MSE 곡선을 평균낸다. 단일 10-겹 실행과 선택된 차수의 안정성을 비교하고, 오차막대와 함께 평균 MSE 곡선을 그려라.
...
**연습문제 5.** <span class="diff med" title="중간"></span> 각 교차검증 방법의 계산비용을 모형 적합 횟수로 논하라. $n$개 관측에 대한 차수 $d$ 다항회귀를 $k$-겹 교차검증으로 평가할 때, 후보 차수의 개수 $D$의 함수로 총 적합 횟수를 표현하고 세 방법을 비교하라.
...
- **세 방법의 성격이 다르다.** 검증집합은 변동이 크고, LOOCV 는 편향이 작지만 분산이 크며, $k$-겹이 균형을 잡는다.
- **13장에서 본 내용이 재표집의 맥락에서 다시 정리된다.** 부트스트랩·순열과 함께 "자료를 다시 써서 불확실성을 재는" 같은 계열이다.
- **다항 차수 선택으로 편향–분산 절충이 눈에 보인다.** 훈련오차는 단조 감소하고 검증오차는 U 자를 그린다.
- **선택의 안정성도 함께 본다.** 겹 나누기를 바꿨을 때 고르는 모형이 달라지면 그 선택이 불안정하다는 뜻이며, **하나의 "최적 모형"을 고집하기 어렵다는 신호**다.
- **누출을 피한다.** 전처리를 각 겹 안에서 해야 한다는 원칙이 여기서도 유효하다.

**이것으로 17장이 끝난다.** 부트스트랩으로 추정하고 순열로 검정하며 교차검증으로 모형을 고르는, **자료를 다시 써서 불확실성을 재는** 세 갈래를 보았다.

다음 장 **정칙화 기법**으로 넘어간다. 설명변수가 많을 때 과대적합을 다스리는 방법이며, 여기서 익힌 교차검증이 조율 모수를 고르는 도구가 된다.
