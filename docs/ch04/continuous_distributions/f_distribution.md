...
??? success "풀이"
    ...
    !!! warning "정규성 가정"
        $F$ 검정은 **정규성에 매우 민감하다.** 모집단이 정규가 아니면 유의수준이 크게 틀어진다. 꼬리가 두꺼운 자료에서는 5% 검정이 실제로는 20%로 작동하기도 한다. 평균 비교($t$ 검정)가 정규성 위반에 꽤 너그러운 것과 대조적이다. 실무에서는 르빈 검정이나 브라운–포사이드 검정처럼 로버스트한 대안을 쓴다.
...
$F$ 검정이 정규성 위반에 민감한 이유를, 분모에 놓인 표본분산의 분포가 모집단의 4차 적률에 의존한다는 사실로 설명하라.
...
??? success "풀이"
    ...
    이고, 여기서 $\mu_4 = E[(X-\mu)^4]$이다. 정규모집단이면 $\mu_4 = 3\sigma^4$이므로 이 식이 $\frac{2\sigma^4}{n-1}$로 줄어들고, 이것이 $\chi^2_{n-1}$의 분산 $2(n-1)$과 정확히 맞아떨어진다.

    **정규가 아니면 이 대응이 깨진다.** 첨도가 큰(꼬리가 두꺼운) 모집단에서는 $\mu_4 > 3\sigma^4$이므로 $S^2$의 분산이 카이제곱이 예측하는 것보다 크다. 그 결과 분자와 분모 모두가 예상보다 크게 흔들리고, 비의 꼬리가 $F$ 분포보다 두꺼워진다. 명목 5% 기각역이 실제로는 15~20%를 기각하는 일이 벌어진다.

    **왜 $t$ 검정은 괜찮은가.** $t$ 통계량의 분자는 표본평균이고, 표본평균은 중심극한정리 덕분에 모집단이 어떻든 정규에 가까워진다. 분모의 $S$도 비율로만 들어가므로 영향이 상쇄되는 쪽으로 작동한다. 반면 $F$ 검정은 **분자와 분모가 모두 2차 적률**이라 중심극한정리의 보호를 받지 못하고 4차 적률에 직접 노출된다.

    **실무 지침.** 등분산 검정으로 $F$ 검정을 쓰지 말라는 권고가 표준이 된 이유가 이것이다. 르빈 검정(편차의 절댓값에 분산분석을 적용)이나 브라운–포사이드 검정(중앙값 기준 편차)이 훨씬 안정적이다. 한 걸음 더 나아가, 애초에 등분산을 검정한 뒤 $t$ 검정을 고르는 2단계 절차 자체가 전체 유의수준을 망가뜨리므로, 처음부터 웰치 $t$ 검정을 쓰는 편이 낫다는 것이 요즘의 권고다.
