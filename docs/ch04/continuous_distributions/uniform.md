...
4.2절의 사슬 $\text{Exp}(\lambda) \to N(\mu,\sigma^2) \to \chi^2_d \to t_d \to F_{d_1,d_2}$ 은 지수분포에서 시작하지만, 그 문을 여는 것은 균등분포다. 아래에서 보는 **역변환 표본추출**을 통해 균등난수 하나로 어떤 분포든 만들 수 있기 때문이다.
