...
정의로 돌아가면 한 줄이다. $Z \sim N(0,1)$에 대해 $E[Z^2] = \text{Var}(Z) = 1$이고, 아래에서 보듯 $E[Z^4] = 3$이므로
...
??? proof "E[Z⁴] = 3의 유도"
    ...
    이 한 줄이 계산 전체를 끌고 간다. 적분에 들어 있는 $x\varphi(x)$를 $-\varphi'(x)$로 바꿔 놓으면 부분적분을 쓸 수 있기 때문이다.

    $$
    E[Z^4] = \int_{-\infty}^{\infty} x^4\,\varphi(x)\,dx
    = \int_{-\infty}^{\infty} x^3 \cdot \big(x\,\varphi(x)\big)\,dx
    = \int_{-\infty}^{\infty} x^3 \cdot \big(-\varphi'(x)\big)\,dx
    $$

    이제 $u = x^3$, $dv = -\varphi'(x)\,dx$로 놓고 부분적분한다.

    $$
    E[Z^4] = \Big[-x^3\varphi(x)\Big]_{-\infty}^{\infty} + \int_{-\infty}^{\infty} 3x^2\,\varphi(x)\,dx
    $$
    ...
    이다. 같은 요령을 되풀이하면 일반항도 나온다. $E[Z^{2k}] = (2k-1)E[Z^{2k-2}]$이므로

    $$
    E[Z^{2k}] = (2k-1)(2k-3)\cdots 3 \cdot 1 = (2k-1)!!
    $$

    이고, 홀수 차수의 적률은 피적분함수가 기함수라 모두 0이다. $k=1,2,3$을 넣으면 차례로 $1, 3, 15$다. $\square$ $Z_i^2$들이 독립이므로 평균과 분산이 각각 더해진다.
