...
- **모수가 $pC$ 개다.** 범주나 특성이 많으면 빠르게 불어나며, MNIST 만 해도 $784\times10$ 개다.
- **18장·19장의 벌점이 그대로 쓰인다.** $L_2$ 가 기본이며 가중치 감쇠라는 이름으로도 불린다.
- **과잉모수 문제를 해결하는 부수 효과가 있다.** 벌점이 있으면 평행이동 자유도가 사라져 해가 유일해진다.
- **편향은 벌점에서 제외한다.** 절편을 벌하면 모형이 기준선을 맞추지 못한다.
- **정칙화가 예측확률을 균등 쪽으로 민다.** 확률값 자체가 필요하면 보정을 고려해야 하며, 19장의 논의가 그대로 적용된다.

다음 절 **소프트맥스 회귀 구현**으로 넘어간다.
