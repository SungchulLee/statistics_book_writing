...
범주별 지표를 합치는 **두 가지 방식**이 있다.

| | 계산 | 성격 |
|---|---|---|
| 거시평균(macro) | 범주별 지표의 단순평균 | **모든 범주를 동등하게** |
| 미시평균(micro) | 전체 $TP$·$FP$·$FN$ 을 합쳐 계산 | **모든 사례를 동등하게** |
| 가중평균 | 범주 크기로 가중 | 큰 범주가 지배 |

- **선택이 결론을 바꾼다.** 희귀 범주의 성능이 나쁘면 거시평균은 낮고 미시평균은 높게 나온다. **어느 쪽을 보고하느냐가 곧 무엇을 중요하게 보느냐다.**
- **미시평균은 다범주에서 정확도와 같아진다.** 각 사례가 정확히 하나의 범주에 속하므로 그렇다.
- **희귀 범주가 중요하면 거시평균을 쓴다.** 의료 진단이나 이상 탐지가 그런 경우다.
- **보고할 때 어느 방식인지 반드시 밝힌다.** 밝히지 않은 "F1 점수"는 해석할 수 없다.
