...
- **$\binom C2$ 개의 분류기가 필요하다.** $C=10$ 이면 45 개로, 일대다의 10 개보다 훨씬 많다.
- **그런데 각각이 작다.** 두 범주의 자료만 쓰므로 개별 학습이 빠르며, **학습 시간이 범주 수에 대해 초선형으로 늘지 않는 경우가 많다.** SVM 처럼 학습 비용이 표본 크기에 초선형인 알고리즘에서 특히 유리하다.
- **예측은 다수결이다.** 각 분류기가 한 표씩 던지며, 동점 처리 규칙이 필요하다.
- **불균형 문제가 덜하다.** 두 범주만 비교하므로 일대다의 구조적 불균형이 없다.
- **관련 없는 분류기도 투표한다는 것이 약점이다.** 실제 범주가 3 인 사례에 대해 "1 대 2" 분류기도 표를 던지며, 그것이 잡음이 된다.

**이것으로 20장이 끝난다.** 이항에서 다범주로의 확장, 소프트맥스와 교차엔트로피, 최적화와 정칙화, 다범주 평가지표, 그리고 일대다·일대일 전략까지 보았다.

다음 장 **생존 모형**으로 넘어간다. 반응이 **사건까지의 시간**이며, 아직 사건이 일어나지 않은 관측을 어떻게 다루느냐가 핵심이다.
